...
Imdb.maketemplate = function (imdb) {
  try {
    var img = `[center][img width="300"]${imdb["Poster"]}[/img][/center]\n`;
  } catch (err) {
    var img = "";
  }
  try {
    var title = `[center][size=150][color=#FF0000][b]${imdb["Title"]} (${imdb["Year"]})[/b][/color][/size][/center]\n`;
  } catch (err) {
    var title = "";
  }
  try {
  ...
  try {
    var director = `[color=#FF8000][b]Director[/b][/color]: ${imdb["Director"]}\n`;
  } catch (err) {
    var director = "";
  }
  try {
    var actors = `[color=#FF8000][b]Stars[/b][/color]: ${imdb["Actors"]}\n`;
  } catch (err) {
    var actors = "";
  }
  try {
    var runtime = `[color=#FF8000][b]Runtime[/b][/color]: ${imdb["Runtime"]} (taken from IMDb)\n`;
  } catch (err) {
    var runtime = "";
  }
  try {
    var genre = `[color=#FF8000][b]Genre[/b][/color]: ${imdb["Genre"]}\n`;
  } catch (err) {
    var genre = "";
  }
  try {
  ...
  try {
    var votes = `[color=#FF8000][b]Votes[/b][/color]: ${imdb["imdbVotes"]} (may differ)\n`;
  } catch (err) {
    var votes = "";
  }
  try {
    var reldate = `[color=#FF8000][b]Release Date[/b][/color]: ${imdb["Released"]} (taken from IMDb)\n`;
  } catch (err) {
    var reldate = "";
  }
  try {
    var vrating = `[color=#FF8000][b]Viewer Rating (TV/MPAA)[/b][/color]: ${imdb["Rated"]} (taken from IMDb)\n`;
  } catch (err) {
    var vrating = "";
  }
  try {
    var summary = `[color=#FF8000][b]Summary[/b][/color]: [i]${imdb["Plot"]}[/i]\n`;
  } catch (err) {
    var summary = "";
  }
  try {
    var links = `[color=#FF8000][b]Links[/b][/color]: `;
  } catch (err) {
    var links = "";
  }
  try {
    var imdburl = `[url=https://www.imdb.com/title/${imdb["imdbID"]}]IMDb ${imdb["imdbID"]}[/url]\n`;
  } catch (err) {
    var imdburl = "";
  }
  try {
...
Imdb.startHandlingImdbAjax = function () {
  ...
  if (match && match[0]) {
    ...
    var url =
      `https://www.omdbapi.com/?apikey=${Imdb.getKey()}&s=` + space2dash(term);
  }
  $ajax = $.ajax({ url: url, dataType: "jsonp" });
  $ajax.done(function (response) {
...
Imdb.handle_imdb = function (response) {
  ...
  for (var entry of aEntry) {
    ...
    if (
      Imdb.getImdbChunkiness(entry) > minChunkiness ||
      (posterUrl && !aExclusion.includes(posterUrl))
    ) {
      $li = $("<li/>").addClass("img_li").appendTo($imdb_content);
