...
Anilist.makeAnilistTemplate = function (data) {
  ...
  var img = Anilist.getEntryOrEmpty(
    `[center][img width="300"]{text}[/img][/center]\n`,
    data["coverImage"]["large"]
  );
  var bannerImage = data["bannerImage"]
    ? Anilist.getEntryOrEmpty(`[banner]{text}[/banner]\n`, data["bannerImage"])
    : "";
  var year = data["startDate"]["year"]
    ? " (" + data["startDate"]["year"] + ")"
    : "";
  var titleNative = Anilist.getEntryOrEmpty(
    `[center][size=200][b][url={url}]{text}${year}[/url][/b][/size][/center]\n`,
    data["title"]["native"],
    url
  );
  var titleRomaji = Anilist.getEntryOrEmpty(
    `[center][size=100][b]{text}[/b][/size][/center]\n`,
    data["title"]["romaji"]
  );
  var titleEnglish = Anilist.getEntryOrEmpty(
    `[center][size=100][b]{text}[/b][/size][/center]\n`,
    data["title"]["english"]
  );
  var rating = Anilist.getRatingOrEmpty(
    `[center][size=150][b]{text} / 10[/b][/size][/center]\n`,
    data["averageScore"]
  );
  var genre = Anilist.getEntryOrEmpty(
    `[center][b][size=120]{text}[/size][/b][/center]\n`,
    data["genres"].join(", ")
  );
  var summary = Anilist.getEntryOrEmpty(
    `[quote][center]{text}[/center][/quote]\n`,
    data["description"]
  );
  var volumes = Anilist.getEntryOrEmpty(
    `[color=#FF8000][b]Volumes[/b][/color]: {text}\n`,
    data["volumes"]
  );
  var format = Anilist.getEntryOrEmpty(
    `[color=#FF8000][b]Format[/b][/color]: {text}\n`,
    Anilist.toTitleCase(data["format"])
  );
  var trailer = "";
  try {
    var trailer =
      "{snahp}{youtube}" + data["trailer"]["id"] + "{/youtube}{/snahp}";
  } catch (e) {}
  var episodes = Anilist.getEntryOrEmpty(
    `[color=#FF8000][b]Episodes[/b][/color]: {text}\n`,
    data["episodes"]
  );
  var endDate = Anilist.getEndDateOrEmpty(
    `[color=#FF8000][b]End Date[/b][/color]: {text}\n`,
    data["endDate"]
  );
  var chapters = Anilist.getEntryOrEmpty(
    `[color=#FF8000][b]Chapters[/b][/color]: {text}\n`,
    data["chapters"]
  );
  var runtime = Anilist.getEntryOrEmpty(
    `[color=#FF8000][b]Runtime[/b][/color]: {text} minutes\n`,
    data["duration"]
  );
  var votes = Anilist.getEntryOrEmpty(
    `[color=#FF8000][b]Votes[/b][/color]: {text}\n`,
...
Anilist.updatePosters = function (media) {
  ...
  for (var entry of media) {
    $li = $("<li/>").addClass("img_li").appendTo($anilist_content);
