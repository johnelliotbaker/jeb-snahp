...
Googlebooks.makeTemplate = function (data) {
  ...
  try {
    var thumbnail = data.imageLinks.thumbnail.replace(
      /zoom=[0-9]+/gi,
      "zoom=3"
    );
  } catch (e) {
  ...
  try {
    var url = data["canonicalVolumeLink"];
  } catch (e) {
    var url = "";
  }
  try {
  ...
  try {
    var language = toTitleCase(data["language"]);
  } catch (e) {
    var language = "";
  }
  try {
  ...
  var title = getEntryOrEmpty(
    `[center][size=200][b][url={url}]{text}[/url][/b][/size][/center]\n`,
    title,
    url
  );
...
Googlebooks.updatePosters = function (media) {
  ...
  for (var entry of media) {
    ...
    try {
      img = vinfo.imageLinks.thumbnail;
    } catch (e) {
      img = "";
    }
    $li = $("<li/>").addClass("img_li").appendTo($googlebooks_content);
  ...
  $('<a href="#googlebooks_dialog" id="modalTriggerLink"></a>').appendTo(
    $("body")
  );
  setTimeout(function () {
...
Googlebooks.handle_googlebooks = function (response, searchTerm) {
  $googlebooks_dialog = $(Googlebooks.googlebooks_dialog_template).appendTo(
    $("body")
  );
  $googlebooks_dialog = $("#googlebooks_dialog");
  $googlebooks_header = $("#googlebooks_header");
  $googlebooks_title = $("#googlebooks_title").text(
...
Googlebooks.startHandlingGooglebooksAjax = function () {
  ...
  if (Array.isArray(match) && match[1]) dict["author"] = match[1];
