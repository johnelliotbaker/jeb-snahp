...
Post_tpl.setup_badges = function () {
  ...
  $.get("/app.php/snahp/template/get/", (resp) => {
    for (var entry of resp) {
      $badge = $(
        `<span class="badge tpl" data-name="${entry["name"]}">${entry["name"]}</span>`
      ).click((e) => {
        $target = $(e.target);
      ...
      $close = $(
        `<button type="button" class="close" data-name="${entry["name"]}"><span aria-hidden="true" data-name="${entry["name"]}">&times;</span></button>`
      )
        .click((e) => {
          $target = $(e.target);
          var name = $target.prop("dataset")["name"];
...
Post_tpl.open = function (name) {
  ...
  if (name == "new") {
    ...
    $.get(
      `/app.php/snahp/template/get/?name=${p["n"]}&full=${p["f"]}`,
      (resp) => {
