...
class api_proxy
{
    ...
    private function respond_goodreads_from_book_id_as_json()
    {
        ...
        $res = json_decode(json_encode((array)simplexml_load_string($body, 'SimpleXMLElement', LIBXML_NOCDATA)), 1); // I know, I'm a terrible human being
        $res = $res['book'];
    ...
    private function respond_goodreads_search_as_json()
    {
        ...
        $arr = [];
        $body = file_get_contents($url);
        $contents = json_decode(json_encode((array)simplexml_load_string($body, 'SimpleXMLElement', LIBXML_NOCDATA)), 1); // I know, I'm a terrible human being
