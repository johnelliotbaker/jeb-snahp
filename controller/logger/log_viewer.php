...
class log_viewer
{
    ...
    public function set_user_spam_config_as_json()
    {
        ...
        if ($buffer_length >= 1) {
            $this->config->set(
                "snp_log_user_spam_buffer_length",
                $buffer_length
            );
        }
        ...
        switch ($type) {
            case "master":
                ...
                return new JsonResponse([
                    "status" => $this->config["snp_log_b_master"],
                ]);
            case "posting":
                $this->config->set("snp_log_b_posting", $b);
                return new JsonResponse([
                    "status" => $this->config["snp_log_b_posting"],
                ]);
            case "viewtopic":
                $this->config->set("snp_log_b_viewtopic", $b);
                return new JsonResponse([
                    "status" => $this->config["snp_log_b_viewtopic"],
                ]);
            case "user_spam":
                $this->config->set("snp_log_b_user_spam", $b);
                return new JsonResponse([
                    "status" => $this->config["snp_log_b_user_spam"],
                ]);
        }
    }
    ...
    public function respond_view($cfg)
    {
        ...
        $this->template->assign_vars([
            ...
            "USER_SPAM_BUFFER_LENGTH" =>
                $this->config["snp_log_user_spam_buffer_length"],
            "TYPE" => $type,
        ]);
