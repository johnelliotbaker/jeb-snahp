...
class acp_reqs extends base
{
    ...
    public function handle_manage_user()
    {
        ...
        $n_use_per_cycle_override = $request->variable(
    ...
    public function test($cfg)
    {
        ...
        return $this->helper->render(
            "@jeb_snahp/acp/resynch_user_requests_solved.html"
        );
    }
    ...
    public function resynch_user_requests_solved($cfg)
    {
        ...
        while ($start < $total) {
            ...
            try {
                ...
                $sql =
                    "SELECT fulfiller_uid, status FROM " .
                    $tbl["req"] .
                    '
                    WHERE status=' .
                    $def["solve"];
                ...
                foreach ($data as $user_id => $entry) {
                    ...
                    $sql =
                        ...
                        USERS_TABLE .
                        '
                        SET snp_req_n_solve=snp_req_n_solve+' .
                        $entry .
                        '
                        WHERE user_id=' .
                        $user_id;
    ...
    {
        $this->reject_non_admin();
        $sid = $this->request->variable(
            $this->config["cookie_name"] . "_sid",
            "",
            true,
        ...
        return $this->helper->render(
            "@jeb_snahp/acp/resynch_request_forums.html"
        );
    }
    ...
    public function resynchronize_requests($b_simulate = false)
    {
        ...
        $result = $this->db->sql_query($sql);
        $graveyard_fid = unserialize($this->config["snp_cron_graveyard_fid"])[
            "default"
        ];
        $res = [];
        if ($b_simulate) {
    ...
    public function update_deleted_requests($uid)
    {
        ...
        $sql =
            "SELECT tid, status FROM " .
            $this->tbl["req"] .
            " WHERE requester_uid=$uid" .
            '
            AND ' .
            $this->db->sql_in_set("status", $this->def["set"]["closed"], true);
        $result = $this->db->sql_query($sql);
    ...
    public function get_banned_dibbers()
    {
        $this->reject_non_moderator();
        $sql =
            "SELECT user_colour, username_clean FROM " .
            USERS_TABLE .
            " WHERE snp_req_dib_enable=0";
        $result = $this->db->sql_query($sql);
        $data = [];
        while ($row = $this->db->sql_fetchrow($result)) {
            $data[] = [
