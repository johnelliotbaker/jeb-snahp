...
class mcp extends base
{
    ...
    public function handle($mode)
    {
        ...
        switch ($mode) {
            case "handle_mass_move_v2":
                $cfg["tpl_name"] =
                    "@jeb_snahp/mcp/component/mcp_mass_move_v2/base.html";
            ...
            case "handle_mass_move":
                $cfg["tpl_name"] =
                    "@jeb_snahp/mcp/component/mcp_mass_move/base.html";
    ...
    public function select_topics_with_search(
        $per_page,
        $start,
        $forum_id = 0,
        $search = ""
    ...
    public function handle_mass_move_v2($cfg)
    {
        ...
        if ($tpl_name) {
            ...
            $request_type = $this->get_cookie_new(
                "mcp",
                "mass_move.request_type"
            );
    ...
    public function select_moderation_topics(
        $per_page,
        $start,
        $forum_id = 0,
        $options = []
        ...
        $b_request = key_exists("b_request", $options)
            ? $options["b_request"]
            : false;
        if ($b_request) {
            $cond_request = " r.tid IS NOT NULL ";
            $request_type = key_exists("request_type", $options)
                ? $options["request_type"]
                : false;
            if ($request_type) {
                $status = key_exists($request_type, $def)
                    ? $def[$request_type]
                    : false;
                if ($status) {
    ...
    public function handle_mass_move($cfg)
    {
        ...
        if ($tpl_name) {
            ...
            // Get topic data
            $start = $this->request->variable("start", 0);
            [$data, $total] = $this->select_moderation_topics(
                $per_page,
                $start,
                $from_fid,
                $options
