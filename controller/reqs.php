...
class reqs extends base
{
    ...
    {
        $this->ptn =
            "#(\(|\[|\{)(accepted|request|closed|fulfilled|solved)(\)|\]|\})\s*#is";
    }
    ...
    public function handle($fid, $tid, $pid, $mode)
    {
        ...
        switch ($mode) {
            ...
            case $def["terminate"]: // To terminate request
                ...
                if (
                    $reqdata["status"] == $def["fulfill"] ||
                    $reqdata["status"] == $def["dib"]
                ) {
                    $cfg["tpl_name"] =
                        "@jeb_snahp/requests/component/terminate_confirm/base.html";
                    $cfg[
                        "base_url"
                    ] = "/viewtopic.php?f={$fid}&t={$tid}#p{$pid}";
                    $cfg["title"] = "Confirm Request Termination";
                    return $this->handle_terminate_confirm(
                        $fid,
                        $tid,
                        $pid,
                        $reqdata,
            ...
            case $def["fulfill"]:
                if (
                    (int) $this->user->data["snp_req_n_solve"] >=
                    $this::SEASONED_SOLVER_THRESHOLD
                ) {
                    $this->fulfill_request(
                        $fid,
                        $tid,
                        $pid,
                        $fulfilledBySeasonedSolver = true
                    );
                    ...
                    $this->fulfill_request(
                        ...
                        $tid,
                        $pid,
                        $fulfilledBySeasonedSolver = false
                    );
                }
                break;
        }
    ...
    public function handle_terminate_confirm($fid, $tid, $pid, $reqdata, $cfg)
    {
        ...
        if ($confirm) {
            $termination_reason = $this->request->variable(
                "termination_reasons",
                ""
            );
            if (!$termination_reason) {
                trigger_error(
                    "You must provide a reason for termination. Error Code: 2a9d306c8a"
                );
            }
            $this->solve_request(
                $fid,
        ...
        if ($tpl_name) {
            ...
            $username =
                '<span style="color: #' .
                $reqdata["fulfiller_colour"] .
                '; font-weight: 900;">' .
                $reqdata["fulfiller_username"] .
                "</span>";
    ...
    public function recalculate_request_users($uid)
    {
        ...
        $n_use = $row["n_use"];
        $new_offset =
            $gdata["snp_req_n_offset"] + $gdata["snp_req_n_base"] - $n_use;
        $data = [
    ...
    public function solve_request(
        ...
        if (!$req) {
            meta_refresh(2, $this->u_action);
            trigger_error(
                "That request does not exist. Error Code: d57e5d63b7"
            );
        }
        ...
        if ($new_status == $def["solve"] && $req["status"] != $def["fulfill"]) {
            meta_refresh(2, $this->u_action);
            trigger_error(
                "You can only solve request that has been fulfilled. Error Code: e4917d39fd"
            );
        }
        ...
        if ($new_status == $def["terminate"]) {
            if (!$this->isTrustedTerminator($req)) {
                ...
                meta_refresh(2, $this->u_action);
                trigger_error(
                    "Only the requester and the moderators are allowed to close this request. Error Code: 585e9279e0"
                );
            }
        ...
        if ($new_status == $def["solve"]) {
            ...
            $sql =
                "UPDATE " .
                $this->tbl["requsr"] .
                " SET n_use_this_cycle=n_use_this_cycle+1 " .
    ...
    public function rejectInvalidTerminator($reqdata)
    {
        if (!$this->isTrustedTerminator($reqdata)) {
            trigger_error(
                "Only the requester and the moderators have access to this page. Error Code: da49eb0a81"
            );
        }
    }
    ...
    public function rejectInvalidSolver($reqdata)
    {
        if (!$this->isTrustedSolver($reqdata)) {
            trigger_error(
                "Only the requester and the moderators have access to this page. Error Code: bfb6ca2540"
            );
        }
    }
    ...
    {
        return $this->isTrustedTerminator($reqdata) ||
            $this->isSeasonedSolver();
    }
    ...
    public function fulfill_request(
        ...
        if ($requesterdata["snp_enable_at_notify"]) {
            ...
            $requester_string = get_username_string(
                "no_profile",
                $requester_id,
                $requester_name,
                $requester_color
            );
            $fulfiller_string = get_username_string(
                "no_profile",
                $fulfiller_id,
                $fulfiller_name,
                $fulfiller_color
            ...
            if ($fulfilledBySeasonedSolver) {
                $text =
                    $fulfiller_string .
                    " solved your request.<br>No further action is required.";
            } else {
                $text =
                    $fulfiller_string .
                    " fulfilled your request<br>Please confirm request resolution.";
            }
    ...
    public function dib_request($fid, $tid, $pid)
    {
            ...
            if ($dibdata["status"] == $def["dib"]) {
                ...
                if ($b_dibber && !$this->b_mod) {
                    if ($this->isSeasonedSolver()) {
                        ...
                        $undib_cooldown =
                            $this->config["snp_req_redib_cooldown_time"];
                    }
        ...
        if ($userId == $topic_data["topic_poster"]) {
            ...
            if ($confirmation !== "true") {
                $cfg = [
                    "tpl_name" =>
                        "@jeb_snahp/confirmation/component/generic/base.html",
                    "title" => "Confirm Request Termination",
                ];
                $this->template->assign_vars([
                    "S_TITLE" =>
                        "Are you sure?<br>You are about to call dibs on your OWN request",
                    "S_MESSAGE" =>
                        "You should do this only when you have found a new resource to your own request and will be posting that resource to the forum.",
