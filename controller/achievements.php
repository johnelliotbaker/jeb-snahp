...
class achievements extends base
{
    ...
    public function handle($mode)
    {
        ...
        if (!$this->config["snp_achi_b_master"]) {
            trigger_error(
                "This achievement system has been disabled by the administrator. Error Code: a089bce09c"
            );
        }
        ...
        switch ($mode) {
            case "update":
                ...
                $cfg["tpl_name"] =
                    "@jeb_snahp/mcp/component/mcp_achievements_update/base.html";
    ...
    public function update($cfg)
    {
        ...
        $this->template->assign_vars([
            ...
            "MEMORY_PEAK_USAGE" => (string) number_format(
    ...
    public function update_thumb_raiser()
    {
        $def = $this->container->getParameter("jeb.snahp.req")["def"];
        $sql =
            "SELECT user_id, snp_thanks_n_received FROM " .
            USERS_TABLE .
    ...
    public function update_life_giver()
    {
        $def = $this->container->getParameter("jeb.snahp.req")["def"];
        $sql =
            "SELECT fulfiller_uid, COUNT(*) as count FROM " .
            $this->tbl["req"] .
            " WHERE status=" .
            $def["solve"] .
            " GROUP BY fulfiller_uid ORDER BY count DESC";
        $result = $this->db->sql_query($sql);
        $row = $this->db->sql_fetchrow($result);
        $this->db->sql_freeresult($result);
        $data = [
    ...
    public function get_powerchart_json($cfg)
    {
        ...
        $user_data = array_map("log", [
            $row["snp_req_n_solve"],
            $row["snp_rep_n_received"],
            $row["snp_thanks_n_received"],
        ]);
    ...
    private function normalize_achievements($data, $reference)
    {
        ...
        foreach ($data as $key => $val) {
            $data[$key] = min(
                max(($max * $data[$key]) / $reference[$key], $min),
                $max
            );
        }
        return $data;
    ...
    private function select_achievements($user_id)
    {
        $user_id = (int) $user_id;
        $sql =
            "SELECT username, snp_req_n_solve, snp_thanks_n_received, snp_rep_n_received FROM " .
            USERS_TABLE .
