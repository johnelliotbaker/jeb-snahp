...
class poi
{
    ...
    private function get_user_data($user_ids)
    {
        if (is_array($user_ids)) {
            ...
            $sql =
                "SELECT user_id, username, user_colour FROM " .
                USERS_TABLE .
                " WHERE ${where}";
    ...
    {
        $sql =
            "SELECT tag, tag_lowercase, count FROM " .
            $this->tbl["topic_tags"] .
            " order by count DESC";
    ...
    public function respondPOI($cfg)
    {
        ...
        foreach ($types as $type) {
            ...
            $imgURL = $typeDetailedData["imgURL"]
                ? $typeDetailedData["imgURL"]
                : "";
            if (!is_string($imgURL)) {
    ...
    public function respond_poi1($cfg)
    {
        ...
        $users_raw = $this->container->getParameter(
            "jeb.snahp.avatar.badge.users"
        );
        $items_raw = $this->container->getParameter(
                ...
                foreach ($queue as $group_name) {
                    if (!in_array($group_name, $excluded_items)) {
                        ...
                        $user = [
                            ...
                            "user_colour" =>
                                $user_db_data[$user_id]["user_colour"],
                            "url" => $url,
                        ];
                        switch ($group_name) {
                            case "encoder":
                                ...
                                $user[
                                    "url"
                                ] = "/search.php?keywords=${tagname}&terms=all&author_id=${user_id}&sc=1&sf=titleonly&sr=topics&sk=x&sd=d&st=0&ch=300&t=0&submit=Search";
                                break;
                            default:
                                break;
                        }
