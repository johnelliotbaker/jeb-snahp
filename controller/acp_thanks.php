...
class acp_thanks extends base
{
    ...
    public function do_resync_all($cfg)
    {
        ...
        $sql = 'SELECT COUNT(DISTINCT user_id) as total FROM ' . $tbl['thanks'] . ' ORDER BY user_id';
        ...
        while($start < $total)
        {
            ...
            catch (\Exception $e)
            {
                ...
                $error_msg = implode('<br>', $this->db->get_sql_error_returned());
                $err = $error_query . '<br>';
                $err .= $error_msg;
                $data = [
        ...
        $sql = 'SELECT COUNT(DISTINCT poster_id) as total FROM ' . $tbl['thanks'] . ' ORDER BY poster_id';
        $result = $this->db->sql_query($sql);
        $row = $this->db->sql_fetchrow($result);
        $this->db->sql_freeresult($result);
        $total = $row['total'];
        ...
        while($start < $total)
        {
            ...
            try
            {
                ...
                foreach($rowset as $row)
                {
                    ...
                    $sql = 'UPDATE ' . USERS_TABLE . ' SET snp_thanks_n_received=' . $n_received . ' WHERE user_id=' . $poster_id;
                    $this->db->sql_query($sql);
                }
                $this->db->sql_freeresult($result);
                if ($this->db->get_sql_error_triggered())
