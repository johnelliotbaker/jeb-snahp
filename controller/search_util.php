...
class search_util extends base
{
    ...
    public function handle($mode)
    {
        ...
        if (!$this->config["snp_inv_b_master"]) {
            trigger_error(
                "Custom search system is disabled by the administrator. Error Code: ce6517a9ac"
            );
        }
        $this->reject_anon();
        switch ($mode) {
            ...
            case "handle_common_words":
                $this->reject_non_dev();
                $cfg["tpl_name"] =
                    "@jeb_snahp/search_util/component/handle_common_words/base.html";
                $cfg["base_url"] =
                    "/app.php/snahp/search_util/handle_common_words/";
            ...
            case "handle_mysql_search":
                $this->reject_non_dev();
                $cfg["tpl_name"] =
                    "@jeb_snahp/search_util/component/handle_mysql_search/base.html";
                $cfg["base_url"] =
                    "/app.php/snahp/search_util/handle_mysql_search/";
    ...
    private function remove_common_word($word)
    {
        $sql =
            ...
            '(word_text, word_common, word_count)
            VALUES ' .
            "('{$word}', 0, 0) " .
            '
    ...
    private function handle_common_words($cfg)
    {
        ...
        if ($tpl_name) {
            ...
            if ($this->request->is_set_post("submit")) {
                ...
                $word_to_search = $this->request->variable(
                    "word_to_search",
                    ""
                );
                $word_to_remove = $this->request->variable(
                    "word_to_remove",
                    ""
                );
                if ($word_to_remove) {
    ...
    {
        $sql =
            "SELECT * from " . SEARCH_WORDLIST_TABLE . " WHERE word_common=1";
    ...
    public function index_topic($cfg)
    {
        ...
        if (!$this->is_search_enhancer_allowed($topic_data)) {
            ...
            trigger_error(
                "Your group does not have the permission to index a topic. Error Code: 8a856e72a8"
            );
        }
        ...
        $error = false;
        $search = new \jeb\snahp\core\fulltext_native_local(
            $error,
            $phpbb_root_path,
            $phpEx,
            $auth,
    ...
    private function markTopicAsSearchEnhance($topicId)
    {
        ...
        $sql =
            "UPDATE " .
            TOPICS_TABLE .
            " SET snp_search_enhance=1 WHERE topic_id=${topicId}";
        $result = $this->db->sql_query($sql);
    }

    private function insert_manual_search_post(
        $post_id,
        $user_id,
        $username_clean
    ) {
