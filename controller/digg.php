...
class digg extends base
{
    ...
    public function handle($mode)
    {
        ...
        if (!$this->config["snp_b_snahp_notify"]) {
            trigger_error(
                "You cannot use digg when notification system is disabled. Error Code: 64be46f11b"
            );
        }
        if (!$this->config["snp_digg_b_master"]) {
            trigger_error(
                "digg system is currently disabled. Error Code: 2e2b753c43"
            );
        }
        if (!$this->config["snp_digg_b_notify"]) {
            trigger_error(
                "digg notification system is currently disabled. Error Code: dcfb0260c2"
            );
        }
        $topic_id = $this->request->variable("t", 0);
        if (!$topic_id) {
            trigger_error(
                "You must provide a topic id. Error Code: 6a46634247"
            );
        }
        ...
        if (!$b_listings) {
            trigger_error(
                "You can only digg in listings. Error Code: 924e331d93"
            );
        }
        ...
        switch ($mode) {
            ...
            default:
                trigger_error(
                    "Invalid request category. Error Code: 57f43ea934"
                );
                break;
        }
    }
    ...
    public function confirm_unregister_digg($topic_id)
    {
        ...
        return $this->helper->render(
            "@jeb_snahp/digg/component/confirm_unregister/base.html"
        );
    }
    ...
    public function unregister_digg($topic_id)
    {
        ...
        if (!$b_op) {
            ...
            trigger_error(
                "Only opening poster can unregister. Error Code: 001ce75c61"
            );
        }
    ...
    public function register_digg($topic_id)
    {
        ...
        if (!$b_op) {
            meta_refresh(4, $this->base_url);
            trigger_error(
                "Only opening poster can register. Error Code: eca16dc835"
            );
        }
        $master_data = $this->select_digg_master($topic_id);
        if ($master_data) {
            meta_refresh(4, $this->base_url);
            trigger_error(
                "You have already registered this digg. Error Code: 28c3e2247b"
            );
        }
    ...
    public function broadcast_digg($topic_id)
    {
        ...
        if (!$b_op) {
            meta_refresh(4, $this->base_url);
            trigger_error(
                "Only opening poster can broadcast. Error Code: 763724b64a"
            );
        }
        $master_data = $this->select_digg_master($topic_id);
        if (!$master_data) {
            meta_refresh(4, $this->base_url);
            trigger_error(
                "You must first register a topic. Error Code: 67b4b00ad1"
            );
        }
        ...
        if (time() < $allowed_time) {
            ...
            trigger_error(
                "You need to wait until " .
                    $this->user->format_date($allowed_time) .
                    " to broadcast this update."
            );
        }
        $this->upsert_digg_master($topic_id);
