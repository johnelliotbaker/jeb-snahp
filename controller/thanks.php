...
class thanks extends base
{
    ...
    public function handle($mode)
    {
        switch ($mode) {
            ...
            case "top_thanks_given":
                ...
                $cfg["tpl_name"] =
                    "@jeb_snahp/thanks/component/top_thanks_given/base.html";
                $cfg["base_url"] =
                    "/app.php/snahp/thanks/handle/top_thanks_given/";
    ...
    public function resp_top_thanks_given($cfg)
    {
        ...
        foreach ($rowset as $row) {
            $row["user_regdate_strn"] = $this->user->format_date(
                $row["user_regdate"]
            );
    ...
    public function handle_thanks_given($cfg)
    {
        ...
        if (!$user_id) {
            trigger_error(
                "You must provide a valid user_id. Error Code: c519466f1a"
            );
        }
        $tpl_name = $cfg["tpl_name"];
