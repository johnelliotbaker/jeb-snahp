...
class wiki
{
    ...
    private function respond_editor()
    {
        ...
        if ($this->request->is_set_post("submit")) {
            ...
            $b_public =
                $this->request->variable("wiki_editor_cb_public", "off") ===
                "on"
                    ? 1
                    : 0;
            if (!$name || !$text) {
    ...
    private function render($cfg)
    {
        ...
        if (!$row) {
            trigger_error(
                "Wikipedia article for <b>${name}</b> does not exist. Error Code: 23a75a95b0"
            );
        }
        $this->b_keepers = $this->sauth->user_belongs_to_groupset(
            $this->user_id,
            "Keepers"
        );
        ...
        if (!$b_public) {
            if (!$this->b_keepers) {
                trigger_error(
                    "You do not have the permission to view this wikipedia article. Error Code: 9888c98c71"
                );
            }
            $b_hidden = true;
        }
