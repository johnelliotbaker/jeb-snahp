...
class acp_fixmediainfo extends base
{
    ...
    private function replace_codebox_with_mediainfo($text)
    {
        ...
        return preg_replace(
            $ptn_storage,
            '<MEDIAINFO><s>[mediainfo]</s>$1<e>[/mediainfo]</e></MEDIAINFO>',
            $text,
            1
        );
    }
    ...
    public function modify_mediainfo($cfg)
    {
        ...
        $sql =
            "SELECT COUNT(*) as total FROM " . TOPICS_TABLE . " WHERE $where";
        ...
        $sql =
            "SELECT * FROM " .
            TOPICS_TABLE .
            " WHERE $where order by topic_id desc";
        $i = 0;
        $this->send_message([
            "status" => "START",
            "i" => $i,
        ...
        try {
            ...
            while ($row = $this->db->sql_fetchrow($result_loop)) {
                ...
                $sql =
                    "SELECT post_text, poster_id, forum_id, post_id, post_subject FROM " .
                    POSTS_TABLE .
