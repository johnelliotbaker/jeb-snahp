...
class economy_user_dashboard
{
    ...
    public function handle($mode)
    {
        switch ($mode) {
            case "user_manager":
                $cfg["tpl_name"] =
                    "@jeb_snahp/economy/mcp/user_manager/base.html";
            ...
            case "overview":
                $cfg["tpl_name"] =
                    "@jeb_snahp/economy/ucp/user_dashboard/base.html";
    ...
    public function handle_overview($cfg)
    {
        ...
        [
            $rowset,
            $total,
        ] = $this->market_transaction_logger->get_user_market_transactions(
            $user_id
        );
        $rowset = array_slice($rowset, 0, 30);
        foreach ($rowset as $entry) {
            $entry["created_time_strn"] = $this->user->format_date(
                $entry["created_time"]
            );
        ...
        [
            $rowset,
            $total,
        ] = $this->bank_transaction_logger->get_user_bank_transactions(
            $user_id
        );
        $rowset = array_slice($rowset, 0, 30);
        foreach ($rowset as $entry) {
        ...
        $inv_count = $this->user_inventory->get_inventory_count_by_product_class(
            $user_id
        );
        ...
        foreach ($product_classes as $entry) {
            if ($entry["name"] == "search_cooldown_reducer") {
                $entry["max_per_user"] = min(
                    $entry["max_per_user"],
                    $this->getMaxSearchInterval($user_id)
                );
            }
            $id = $entry["id"];
            $entry["n_bought"] = array_key_exists($id, $inv_count)
                ? $inv_count[$id]
                : 0;
            $entry["b_buy"] = $entry["n_bought"] < $entry["max_per_user"];
            $entry["max_purchasable"] =
                $entry["max_per_user"] - $entry["n_bought"];
    ...
    {
        return (int) $this->market->get_total_cost(
            $product_class_id,
            $quantity
        );
    }
    ...
    private function can_buy($required_quantity, $product_class_id, $user_id)
    {
        ...
        if ($required_quantity > $purchasable_quantity) {
            return [
                0,
                "You cannot purchase more than ${purchasable_quantity}.",
            ];
        }
    ...
    public function handle_buy_product($cfg)
    {
        ...
        [$b_purchasable, $reason] = $this->can_buy(
            $quantity,
            $product_class_id,
            $user_id
        );
        ...
        if (!$b_bank) {
            return $js->send([
                "status" => 0,
                "reason" => "Could not process banking transaction.",
            ]);
        } else {
            $this->bank_user_account->log_withdraw(
                $total_cost,
                $user_id,
                $broker_id = -1
            );
        ...
        $b_inv = $this->user_inventory->do_add_item_with_logging(
            $product_class_id,
            $quantity,
            $user_id
        );
        if (!$b_inv) {
            return $js->send([
                "status" => 0,
                "reason" => "Could not add inventory to the user.",
            ]);
        }
        return $js->send(["status" => 1, "reason" => "Success"]);
    }
    ...
    public function handle_exchange($cfg)
    {
        ...
        [$b_success, $reason] = $this->bank_user_account->do_exchange(
            $exchange_rate_id,
            $direction,
            $amount,
            $user_id
