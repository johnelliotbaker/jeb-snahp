...
class economy_dashboard
{
    ...
    public function handle($mode)
    {
        switch ($mode) {
            case "overview":
                $cfg["tpl_name"] =
                    "@jeb_snahp/economy/mcp/economy_dashboard/base.html";
    ...
    private function get_bank_transaction_data_for_pagination($start, $per_page)
    {
        ...
        foreach ($rowset as &$row) {
            $row["created_time"] = $this->user->format_date(
                $row["created_time"]
            );
        }
        return [$rowset, $total];
        ...
        switch ($type) {
            case "bank":
                [
                    $rowset,
                    $total,
                ] = $this->get_bank_transaction_data_for_pagination(
                    $start,
                    $per_page
                );
            ...
            case "market":
                [
                    $rowset,
                    $total,
                ] = $this->get_market_invoice_data_for_pagination(
                    $start,
                    $per_page
                );
