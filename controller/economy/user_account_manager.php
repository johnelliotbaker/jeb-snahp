...
class user_account_manager
{
    ...
    public function handle_user_manager($cfg)
    {
        return $this->helper->render(
            $cfg["tpl_name"],
            "Snahp Economy User Account Manager"
        );
    }
    ...
    private function handle_set_user_balance()
    {
        ...
        if ($b_success) {
            $this->bank_user_account->log_moderation(
                $balance,
                $user_id,
                $broker_id
            );
    ...
    {
        return $this->bank_user_account->set_balance(
            $user_id,
            $balance,
            $broker_id
        );
    ...
    private function handle_set_user_inventory_item()
    {
        ...
        $js = new \phpbb\json_response();
        $broker_id = $this->user_id;
        $b_success = $this->user_inventory->set_item_quantity(
            $product_class_id,
            $quantity,
            $user_id,
            $broker_id
        ...
        if ($b_success) {
            ...
            $this->user_inventory->log_moderation(
                $product_class_id,
                $quantity,
                $user_id,
                $broker_id,
    ...
    private function set_user_inventory_item()
    {
        ...
        $js = new \phpbb\json_response();
        $broker_id = $this->user_id;
        $this->user_inventory->set_item_quantity(
            $product_class_id,
            $quantity,
            $user_id,
            $broker_id
