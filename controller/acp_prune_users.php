...
class acp_prune_users extends base
{
    ...
    public function deactivate($cfg)
    {
        ...
        $sql =
            "SELECT user_id, username, user_type, user_posts, group_id, user_lastvisit FROM " .
            USERS_TABLE;
    ...
    public function prune($cfg)
    {
        ...
        if (!$time_strn) {
            trigger_error(
                "You must specify cut off date e.g. ?time=2018-10-30"
            );
        }
        ...
        if ($process_doa == 1) {
            ...
            $cfg["time"] = $time;
            $cfg["detail"] =
                "Prune users that never signed in after user creation";
        ...
        // Prune users that are inactive
            ...
            $cfg[
                "detail"
            ] = "Prune users that did not login after {$time_strn}";
    ...
    public function prune_users($cfg)
    {
        ...
        while ($user_ids = $this->get_users_inactive_after($cfg)) {
            if ($n_loop > 0 && $i_loop >= $n_loop) {
                $this->send_message([
                    "status" => "BREKAING due to n_loop",
                    "i" => $cfg["limit"] * ($i_loop + 1),
                ]);
                break;
            }
            if (count($user_ids)) {
    ...
    private function get_users_inactive_after_count($cfg)
    {
        ...
        $limit = $cfg["limit"];
        $process_doa =
            array_key_exists("process_doa", $cfg) && $cfg["process_doa"];
