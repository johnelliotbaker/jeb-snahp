...
class rh_tags_list
{
    ...
    public function handle($mode)
    {
        switch ($mode) {
            case "whitelist":
                $cfg["tpl_name"] =
                    "@jeb_snahp/rh_tags/component/whitelist/base.html";
                $cfg["title"] = "RH Tags Whitelist";
                return $this->respond_whitelist($cfg);
            case "forum_map":
                $cfg["tpl_name"] =
                    "@jeb_snahp/rh_tags/component/forum_map/base.html";
                $cfg["title"] = "RH Tags Forum Map";
                return $this->respond_forum_map($cfg);
            case "group_map":
                $cfg["tpl_name"] =
                    "@jeb_snahp/rh_tags/component/group_map/base.html";
    ...
    public function respond_forum_map($cfg)
    {
        ...
        if ($tpl_name) {
            ...
            if ($this->request->is_set_post("add")) {
                ...
                foreach ($a_forum_id as $forum_id) {
                    ...
                    while (
                        ...
                        $entry["priority"] = $this->request->variable(
                            "priority_{$index}",
                            0
                        );
                        $entry["groupname"] = $tmp;
                    ...
                    $this->tags_helper->get_groupnames_from_forum_map(
                        $forum_id,
                        0
                    );
                }
            ...
            $u_action = $this->helper->route("jeb_snahp_routing.rh_tags_list", [
                "mode" => "forum_map",
            ]);
        ...
        if ($tpl_name) {
            ...
            if ($this->request->is_set_post("add")) {
                ...
                if ($groupname = $this->request->variable("groupname", "")) {
                    ...
                    $this->tags_helper->get_tagnames_from_group_map(
                        $groupname,
                        0
                    );
                }
    ...
    public function respond_whitelist($cfg)
    {
        ...
        if ($tpl_name) {
            ...
            if ($this->request->is_set_post("submit")) {
                ...
                $whitelist = $this->convert_whitelist_for_config_text(
                    $whitelist
                );
                $this->config_text->set($this->whitelist_cfg_name, $whitelist);
            }
            $data = stripslashes(
                $this->config_text->get($this->whitelist_cfg_name)
            );
            if ($data) {
                if ($data[0] != "[" || $data[-1] != "]") {
                    trigger_error(
                        "Invalid RT Whitelist. Error Code: 694fe6cc77"
                    );
                }
    ...
    private function make_forum_tag_tree()
    {
        ...
        foreach ($a_forum_data as $forum_data) {
            ...
            $arr = array_map(function ($arg) {
                return $arg["groupname"] . " ({$arg["priority"]})";
            }, $rowset);
    ...
    private function get_sub_forums()
    {
        ...
        if (sizeof($allowed_forum_ids) > 0) {
            $sql =
                ...
                '
                    WHERE ' .
                $this->db->sql_in_set("forum_id", $allowed_forum_ids) .
                " AND forum_type <> " .
                FORUM_LINK .
                '
            ...
            while ($row = $this->db->sql_fetchrow($result)) {
                if (
                    (int) $row["parent_id"] != (int) end($parent_stack) ||
                    end($parent_stack) == 0
                ) {
