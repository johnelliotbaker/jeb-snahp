...
class reqs_post extends base
{
    ...
    public function handle_thanks()
    {
        ...
        $sql =
            "SELECT post_id, topic_id from phpbb_posts order by post_time desc";
    ...
    public function handle_request_form($cfg, $fid)
    {
        ...
        if ($tpl_name) {
            ...
            if ($this->request->is_set_post("submit")) {
                ...
                $vars = ["message", "forum_id"];
                extract(
                    $phpbb_dispatcher->trigger_event(
                        "jeb.snahp.request_post_submit_before",
                        compact($vars)
                    )
                );
                $message_parser = new \parse_message();
                ...
                $message_parser->parse(
                    $data_ary["enable_bbcode"],
                    $this->config["allow_post_links"]
                        ? $data_ary["enable_urls"]
                        : false,
                    $data_ary["enable_smilies"],
                    true,
                    true,
                    true,
                    $this->config["allow_post_links"]
                ...
                trigger_error(
                    $this->user->lang("ACP_SNP_SETTING_SAVED") .
