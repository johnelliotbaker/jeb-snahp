...
class favorite extends base
{
    ...
    public function handle_accepted_requests($cfg)
    {
        ...
        if ($tpl_name) {
            ...
            if ($type == "dib") {
                ...
                [$data, $total] = $this->select_accepted_requests(
                    $this->perPage,
                    $start,
                    "dib"
                );
    ...
    public function handle_open_requests($cfg)
    {
        ...
        if ($tpl_name) {
            ...
            if ($type == "fulfill") {
                ...
                [$data, $total] = $this->select_fulfilled_requests(
                    $this->perPage,
                    $start
                );
                $base_url .= "?type=fulfill";
            } else {
                [$data, $total] = $this->select_open_requests(
                    $this->perPage,
                    $start
                );
            }
    ...
    public function handle_thanks_given($cfg)
    {
        ...
        if ($tpl_name) {
            ...
            $pagination = $this->container->get("pagination");
            $start = $this->request->variable("start", 0);
            [$data, $total] = $this->select_thanks_given(
                $this->perPage,
                $start
            );
            $pagination->generate_template_pagination(
    ...
    public function handle_favorite($cfg)
    {
        ...
        if ($tpl_name) {
            ...
            if ($b_dev) {
                $a_img_url = $this->container->getParameter("jeb.snahp.fav")[
                    "dev"
                ]["img"];
            } else {
                $a_img_url = $this->container->getParameter("jeb.snahp.fav")[
            ...
            if ($this->request->is_set_post("submit")) {
                ...
                $a_target_forum_full_depth = $this->select_subforum(
                    $fid_listings,
                    $cooldown
                );
                $exclude = [];
                foreach ($a_target_forum_full_depth as $fid) {
                    $b_exclude = !$this->request->variable(
                        "fav_filter_fid_" . $fid,
                        false
                    );
                    if ($b_exclude) {
            ...
            $a_forum = [];
            $a_listings_forum_parent = $this->select_subforum_with_name(
                $fid_listings,
                $cooldown,
                true
            );
            ...
            foreach ($a_listings_forum_parent as $forum) {
                ...
                $rowset = $this->select_subforum_with_name(
                    $parent_id,
                    $cooldown
                );
                if ($rowset) {
