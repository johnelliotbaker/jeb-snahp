...
class ReporterPermission
{
    ...
    public function hasPermission($request, $userId, $kwargs = [])
    {
        ...
        switch ($method) {
            case "GET":
                if (
                    (int) $topicData["topic_poster"] ===
                        (int) $this->sauth->userId ||
                    $this->sauth->is_dev()
                ) {
                    return true;
                }
    ...
    public function canCreateRequest($topicId)
    {
        if (!$this->Entry->isAuthor($topicId, $this->sauth->userId)) {
            throw new \Exception(
                "You do not have the permission to access this page. Error Code: 90d17e3c3f"
            );
        }
        $entry = $this->Entry->activeEntry($topicId);
    ...
    public function canCreateAction($topicId)
    {
        if (!$this->sauth->is_dev()) {
            throw new \Exception(
                "You do not have permission to view this page. Error Code: c203866c4f"
            );
        }
        $entry = $this->Entry->activeEntry($topicId);
        if ($entry->type !== "Request") {
...
class EntryListCreateAPIView extends ListCreateAPIView
{
    ...
    public function checkPermissions($request, $userId, $kwargs = [])
    {
        ...
        if (!array_key_exists("topicId", $kwargs)) {
            ...
            if (!isset($data["topic"]) || !isset($data["type"])) {
                throw new \Exception(
                    "Required data not found. Error Code: 17bf6d9b81"
                );
            }
            $kwargs = [
                "topicId" => $data["topic"],
    ...
    public function viewByTopicId($topicId)
    {
        ...
        if (!$topicData) {
            throw new \Exception(
                "That topic does not exist. Error Code: 334c9082e4"
            );
        }
        $this->checkPermissions($this->request, $this->sauth, [
            "topicId" => $topicId,
            "topicData" => $topicData,
        ]);
...
class RequestDoesNotExist extends \Exception
{
    public function __construct(
        $strn = "Request does not exist. Error Code: f52224f58d"
    ) {
...
{
    public function __construct(
        $strn = "Report does not exist. Error Code: 1169cbc3e1"
    ) {
        parent::__construct($strn);
    }
}
...
{
    public function __construct(
        $strn = "This topic has a pending action. Error Code: 71426ecd63"
    ) {
        parent::__construct($strn);
    }
}
...
{
    public function __construct(
        $strn = "This topic has a pending request. Error Code: 05b7d48d54"
    ) {
        parent::__construct($strn);
    }
}
...
{
    public function __construct(
        $strn = "This topic has a pending report. Error Code: bf583cf637"
    ) {
        parent::__construct($strn);
    }
}
