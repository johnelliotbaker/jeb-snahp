...
class DeadlinksEventListener implements EventSubscriberInterface
{
    ...
    protected $sauth;
    protected $Entry;
    public function __construct($template, $sauth, $Entry, $helper)
    {
    ...
    {
        return [
            "core.user_setup_after" => [["showDeadlinks", 1]],
            ...
            "core.search_modify_tpl_ary" => [["setDeadlinksTagInSearch", 10]],
        ];
    }
    ...
    public function setupSearchParams($event)
    {
        ...
        $event["ex_fid_ary"] = $this->helper->appendGraveyardToExFidAry(
            $event["ex_fid_ary"]
        );
    }
    ...
    public function showDeadlinks($event)
    {
        ...
        $openReports = count(
            $this->Entry->getOpenReports($this->sauth->userId)
        );
    ...
    public function setDeadlinkTagInTitle($event)
    {
        ...
        if ($isDeadlink) {
            $topic_data["topic_title"] =
                "[Deadlinks] " . $topic_data["topic_title"];
        }
