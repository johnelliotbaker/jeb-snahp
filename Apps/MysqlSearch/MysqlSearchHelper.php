...
class MysqlSearchHelper
{
    ...
    public function userIsPermitted($userId)
    {
        ...
        return $this->sauth->user_belongs_to_groupset($userId, "Staff") ||
            $this->sauth->user_belongs_to_group(
                $userId,
                $groups["Elite Users"]["id"]
            );
    }
    ...
    public function rejectUnauthorizedAccess()
    {
        if (!$this->userIsPermitted($this->userId)) {
            throw new UserPermissionError(
                "You are not allowed to access this feature. Error Code: 84ceefc58a"
            );
        }
    }
    ...
    public function filterForums($sqlAry, $forumType)
    {
        ...
        foreach ($allowedTypes as $type) {
            ...
            $subForums = array_merge(
                $subForums,
                $this->forumHelper->selectSubforum($rootForum)
            );
        }
        if (isset($subForums)) {
    ...
    public function getRootForumId($type)
    {
        switch ($type) {
            ...
            default:
                throw new InvalidForumTypeError(
                    "$type is not supported. Error Code: c39f058c90"
                );
        }
    }
    ...
    public function rejectFloodingRequests()
    {
        ...
        if ($flooding) {
            ...
            throw new SearchFloodingError(
                "You searching too quickly. Will automatically redirect in {$left} seconds. Error Code: 0c4b39d13a"
            );
        }
    }
    ...
    public function userIsFlooding($userId)
    {
        ...
        if (!$row) {
            throw new InvalidUser(
                "That user does not exist. Error Code: c7ce244486"
            );
        }
    ...
    public function updateUserLastSearchTime($userId)
    {
        ...
        $sql =
            "UPDATE " .
            USERS_TABLE .
            " SET user_last_search=${time} WHERE user_id=${userId}";
