...
require_once "/var/www/forum/ext/jeb/snahp/Apps/Core/Pagination/Page.php";
...
class DjangoPaginator
{
    ...
    public function validateNumber($number)
    {
        try {
            ...
            throw new PageNotAnInteger(
                "That page number is not an integer. Error Code: 77bbea4e67"
            );
        }
        if ($number < 1) {
            throw new EmptyPage(
                "That page number is less than 1. Error Code: d3d1499052"
            );
        }
        if ($number > $this->numPages()) {
            if ($number == 1 && $this->allowEmptyFirstPage) {
                ...
                throw new EmptyPage(
                    "That page contains no results. Error Code: 279bcc5efc"
                );
            }
        }
        return $number;
    ...
    public function page($number)
    {
        ...
        return $this->_getPage(
            $this->objectList->slice($bottom, $this->perPage),
            $number,
            $this
        );
    }
