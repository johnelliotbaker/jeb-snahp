...
class PostingViolationController
{
    ...
    protected $sauth;
    protected $helper;
    public function __construct($config, $request, $template, $sauth, $helper)
    {
        $this->config = $config;
    ...
    {
        $results = $this->helper->getPostingViolationsUserToplist(
            $this->request
        );
    ...
    {
        $results = $this->helper->getUserPostingViolations(
            $username,
            $this->request
        );
        return new JsonResponse($results);
