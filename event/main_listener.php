...
class main_listener extends base implements EventSubscriberInterface
{
    ...
    public static function getSubscribedEvents()
    {
        return [
            ...
            "core.viewtopic_cache_user_data" => [["shorter_join_date", 1]],
            ...
            "core.modify_posting_auth" => [["block_zebra_foe_quote", 0]],
            ...
            "core.search_modify_interval" => [["modify_search_interval", 0]],
            "core.ucp_pm_compose_modify_data" => [["set_pm_mode", 0]],
            "core.decode_message_before" => [["censor_hide_in_pm", 0]],
        ];
    }
    ...
    public function reinsert_code_bbcode($text, $ptn_uuid, $codebox_matches)
    {
        ...
        $new_text = "";
        while (
            preg_match($ptn_uuid, $text, $matches, PREG_OFFSET_CAPTURE, $start)
        ) {
    ...
    public function process_base64_bbcode($event)
    {
        ...
        $text = preg_replace_callback(
            $ptn,
            function ($matches) use ($permission) {
                if ($permission) {
    ...
    public function colorize_staff_notification($event)
    {
        ...
        if ($this->is_dev_server()) {
            $group_def = $this->container->getParameter("jeb.snahp.groups")[
                "dev_staff_notification_alias"
            ];
        } else {
            $group_def = $this->container->getParameter("jeb.snahp.groups")[
                "production_staff_notification_alias"
            ];
        }
        $a_group = array_map(function ($arg) use ($group_def) {
            return $arg != "set" && isset($group_def[$arg])
                ? $group_def[$arg]
                : null;
        }, $a_group);
    ...
    public function modify_user_rank($event)
    {
        ...
        $product_class_data = $this->product_class->get_product_class_by_name(
            (string) $product_class_name
        );
        if ($product_class_data) {
            $pcid = (int) $product_class_data["id"];
            $inv_data = $this->user_inventory->get_single_inventory(
            ...
            if ($inv_data) {
                ...
                if ($rank_img) {
                    $post_row["RANK_IMG"] =
                        '<img style="display: block; margin-top: 4px; max-height: 40px; max-width: 160px;" src="' .
                        $rank_img .
                        '"/>';
                }
                if ($rank_title) {
                    $post_row[
                        "RANK_TITLE"
                    ] = "<span style='font-size: 1.1em; display: inline-block; margin-top: 4px;'>$rank_title</span>";
                }
            }
        }
        ...
        if ($this->config["snp_search_b_enable"]) {
            $interval =
                $user_id == ANONYMOUS
                    ...
                    : $this->sauth->getMinFromGroupMemberships(
                        $user_id,
                        "snp_search_interval"
                    );
        }
    ...
    public function setup_viewtopic_modify_post_row_data($event)
    {
        ...
        $sql =
            "SELECT * FROM " .
            $tbl["reputation"] .
            " WHERE giver_id={$user_id} AND post_id={$post_id}";
        $result = $this->db->sql_query($sql);
        $row = $this->db->sql_fetchrow($result);
        $this->db->sql_freeresult($result);
    ...
    {
        $sql =
            "SELECT snp_rep_n_received, snp_thanks_n_received, snp_req_n_solve FROM " .
            USERS_TABLE .
    ...
    public function show_reputation_in_profile($event)
    {
        ...
        $sql_array = [
            "SELECT" =>
                "r.post_id, p.post_subject, p.topic_id, p.forum_id, COUNT(*) as count",
    ...
    public function shorter_join_date($event)
    {
        ...
        $user_cache_data["joined"] = $this->user->format_date(
            $row["user_regdate"],
            "M d, Y"
        );
    ...
    public function show_reputation_in_avatar($event)
    {
        ...
        if (!$this->b_rep_given) {
            $post_row["SHOW_REPUTATION_BTN"] =
                $this->user->data["user_id"] == $poster_id ? false : true;
        }
        $event["post_row"] = $post_row;
    }
    ...
    public function show_achievements_in_profile($event)
    {
        ...
        $params = $this->container->getParameter(
            "jeb.snahp.avatar.achievements"
        );
    ...
    public function show_achievements_in_avatar($event)
    {
        ...
        $title = preg_replace('#s\'s#', 's\'', $title);
        $post_row["U_AVATAR_ACHIEVEMENTS_IMG"] =
            $params[$type]["img"]["url"][$style_name];
    ...
    {
        if (
            !$this->config["snp_b_snahp_notify"] ||
            !$this->config["snp_digg_b_master"] ||
            !$this->config["snp_digg_b_notify"]
        ) {
            return false;
        }
        $cooldown = 2;
        ...
        if ($master_data) {
            $b_digg_master = true;
            $slave_data = $this->select_digg_slave_count(
                $topic_id,
                $cooldown = $cooldown
            );
        ...
        if (!$b_op) {
            if ($b_digg_master) {
                $state = 2;
                $slave_data = $this->select_digg_slave(
                    $topic_id,
                    $where = "user_id={$user_id}"
                );
                if ($slave_data) {
    ...
    public function show_thanks_given_in_profile($event)
    {
        if ($this->is_dev()) {
            ...
            $sql =
                "SELECT snp_thanks_n_given FROM " .
                USERS_TABLE .
    ...
    public function setup_core_vars($event)
    {
        ...
        $this->template->assign_vars([
            ...
            "B_RED_TEAM" => $this->user_belongs_to_groupset(
                $this->user_id,
                "Red Team"
            ),
        ]);
    ...
    public function replace_with_host_icons_in_listings($event)
    {
        ...
        foreach ($rowset as $key => $row) {
            $row["topic_title"] = $this->encodeTags($row["topic_title"]);
    ...
    public function show_thanks_for_op($event)
    {
        ...
        $thanks_total = 0;
        if (
            $this->poster_data["snp_thanks_b_topic"] ||
            $this->is_op($topic_data) ||
            $this->is_dev()
        ) {
    ...
    public function set_is_listings($event)
    {
        ...
        if (!array_key_exists("isListings", $this->cache)) {
            $this->cache["isListings"] = $this->is_listing(
                $topic_data,
                "topic_data"
            );
        }
    ...
    public function block_zebra_foe_quote($event)
    {
        ...
        if ($row && $mode == "quote") {
            trigger_error(
                "Sorry, quoting in this topic is currently disabled."
            );
        }
    }
    ...
    public function block_zebra_foe_topicview($event)
    {
        ...
        if ($row) {
            ...
            $username =
                ...
                "</span>";
            $post_row["MESSAGE"] =
                "Sorry, this post is currently unavailable for viewing.";
            $event["post_row"] = $post_row;
        }
    }
    ...
    public function disable_quoting_blocker($event)
    {
        ...
        $foe_blocker_helper = $this->container->get(
            "jeb.snahp.foe_blocker_helper"
        );
        if (
            $foe_blocker_helper->is_blocked_with_blocker_username(
                $this->user_id,
                $username
            )
        ) {
            trigger_error(
                "You have been blocked by <b>${username}</b> and cannot quote. Error Code: 299a70edf4"
            );
        }
    }
    ...
    public function show_thanks_top_list($event)
    {
        ...
        $sql =
            "SELECT user_id, username, user_colour, snp_thanks_n_received FROM " .
            USERS_TABLE .
            '
    ...
    public function giftbox($event)
    {
        ...
        $post_row = $event["post_row"];
        if (
            $post_row["POST_ID"] != $this->config["snp_giv_injection_post_id"]
        ) {
            return;
        }
        $start_html = "";
        $end_html = "";
        $post_row["MESSAGE"] =
            $start_html . $html . $end_html . $post_row["MESSAGE"];
        $event["post_row"] = $post_row;
    }
    ...
    public function easter_cluck($event)
    {
        ...
        $motd_message =
            "Hello " .
            $username .
            ", thank you very much for being an important member of our community.<br>";
        $topic_data = $event["topic_data"];
        $tt = $topic_data["topic_title"];
        $strn = string2cluck($strn);
        $post_row["MESSAGE"] = $motd_message . $strn;
        $post_row["MESSAGE"] .=
            '<p style="font-size:.5em; text-align: center;">This is an easter egg</p>';
        $event["post_row"] = $post_row;
    }
    ...
    private function preg_replace_emotes($message)
    {
        ...
        foreach ($a_match[1] as $keyword) {
            if (array_key_exists($keyword, $emotes)) {
                ...
                $css = array_key_exists("css", $emote)
                    ? " " . $emote["css"]
                    : "";
                $title = array_key_exists("title", $emote)
                    ? " " . $emote["title"]
                    : "";
                $link = array_key_exists("link", $emote)
                    ? " " . $emote["link"]
                    : "";
    ...
    {
        $user_id = $this->config["snp_ql_your_topics"]
        ...
        $this->template->assign_vars([
            ...
            "B_SHOW_ACCEPTED_REQUESTS" =>
                $this->config["snp_ql_req_accepted_requests"],
            "S_USER_ID" => $user_id,
        ]);
    }
    ...
    {
        $this->template->assign_vars([
            "B_SHOW_DONATION_NAVLINK" =>
                $this->config["snp_don_b_show_navlink"],
            "DON_URL" => $this->config["snp_don_url"],
        ]);
    }

    public function get_user_string_from_usernames_sql(
        $aUserdata,
        $prepend = "",
        $bDullBlocked = false
    ) {
        ...
        foreach ($aUserdata as $key => $row) {
            ...
            $username_string =
                "[color=#$color][b]$prepend$username" . "[/b][/color]";
    ...
    public function modify_dice_roll($event)
    {
        ...
        $b =
            "[center][fimg=250,250]" .
            "https://raw.githubusercontent.com/codexologist/img/master/img/roll/ishihara_" .
            $num .
            ".png" .
            "[/fimg][/center]";
        $message = preg_replace($a, $b, $message);
    ...
    public function colorize_at($event)
    {
        ...
        foreach ($aUserdata as $userdata) {
            if (
                $foe_blocker_helper->is_blocked_with_blocker_id(
                    $this->user_id,
                    $userdata["user_id"]
                )
            ) {
                trigger_error(
                    "You have been blocked by <b>${userdata["username"]}</b> and cannot send poke notifications. Error Code: 769a81bcab"
                );
            }
        }
        $aUserString = $this->get_user_string_from_usernames_sql(
            $aUserdata,
            $at_prefix,
            true
        );
    ...
    public function notify_on_poke($event)
    {
        ...
        $username_string = get_username_string(
            "no_profile",
            $user_id,
            $username,
            $color
        ...
        foreach ($aReceiverData as $row) {
            ...
            $receiver_string = get_username_string(
                "no_profile",
                $receiver_id,
                $receiver_name,
                $receiver_color
    ...
    public function include_assets_before_posting($event)
    {
        ...
        switch ($mode) {
            ...
            case "edit":
                ...
                $sql =
                    "SELECT topic_first_post_id FROM phpbb_topics WHERE topic_id=" .
                    (int) $topic_id;
                $result = $this->db->sql_query($sql);
                $row = $this->db->sql_fetchrow($result);
                $this->db->sql_freeresult($result);
        ...
        $sql =
            'SELECT snp_imdb_enable, snp_anilist_enable,
        ...
        if ($b_first_post) {
            ...
            foreach ($pg_names as $pg_name) {
                $fid_allowed[$pg_name] = explode(
                    ",",
                    $this->config["snp_pg_fid_" . $pg_name]
                );
            }
            if (
                $row["snp_imdb_enable"] &&
                in_array($forum_id, $fid_allowed["listing"])
            ) {
                $this->template->assign_vars(["B_SHOW_IMDB" => true]);
            }
            if (
                $row["snp_anilist_enable"] &&
                in_array($forum_id, $fid_allowed["anime"])
            ) {
                $this->template->assign_vars(["B_SHOW_ANILIST" => true]);
            }
            if (
                $row["snp_googlebooks_enable"] &&
                in_array($forum_id, $fid_allowed["book"])
            ) {
                $this->template->assign_vars(["B_SHOW_BOOKS" => true]);
            }
            if (
                $row["snp_gamespot_enable"] &&
                in_array($forum_id, $fid_allowed["game"])
            ) {
                $this->template->assign_vars(["B_SHOW_GAMES" => true]);
            }
            if (
                $row["snp_mydramalist_enable"] &&
                in_array($forum_id, $fid_allowed["mydramalist"])
            ) {
                $this->template->assign_vars(["B_SHOW_MYDRAMALIST" => true]);
            }
            if (
                $row["snp_discogs_enable"] &&
                in_array($forum_id, $fid_allowed["discogs"])
            ) {
    ...
    public function disable_signature($event)
    {
        ...
        $sql =
            "SELECT snp_enable_signature from " .
            GROUPS_TABLE .
            " WHERE group_id =" .
            $gid;
        $result = $this->db->sql_query($sql, 30);
    ...
    public function modify_signature($event)
    {
        ...
        $gid = $this->user->data["group_id"];
        $sql =
            "SELECT snp_signature_rows from " .
            GROUPS_TABLE .
            " WHERE group_id =" .
            $gid;
        $result = $this->db->sql_query($sql, 30);
