...
class request_listener extends base implements EventSubscriberInterface
{
    ...
    public static function getSubscribedEvents()
    {
        return [
            ...
            "core.viewtopic_assign_template_vars_before" => [
                ["show_request_icons", 1],
                ["show_dibs", 0],
            ],
            "core.posting_modify_template_vars" => [
                ["disable_user_posting", 0],
            ],
            ...
            "core.delete_topics_after_query" =>
                "update_request_user_after_topic_deletion",
    ...
    public function include_reqs_forum_assets($event)
    {
        ...
        if (!in_array($forum_id, $request_fid)) {
            return false;
        }
        $this->template->assign_vars([
    ...
    public function show_fulfillment_stats($event)
    {
        ...
        $sql =
            ...
            $tbl .
            " WHERE fid={$forum_id} AND status={$def["open"]}";
        ...
        $sql =
            "SELECT COUNT(*) as total FROM " .
            $tbl .
            " WHERE fid={$forum_id} AND status={$def["dib"]}";
        $result = $this->db->sql_query($sql, $cache_cooldown);
        $row = $this->db->sql_fetchrow($result);
        $this->db->sql_freeresult($result);
        $n_dib = $row["total"];
        ...
        $sql =
            "SELECT COUNT(*) as total FROM " .
            $tbl .
            " WHERE fid={$forum_id} AND status={$def["fulfill"]}";
        $result = $this->db->sql_query($sql, $cache_cooldown);
        $row = $this->db->sql_fetchrow($result);
        $this->db->sql_freeresult($result);
        $n_fulfill = $row["total"];
        ...
        $sql =
            "SELECT COUNT(*) as total FROM " .
            $tbl .
            " WHERE fid={$forum_id} AND status={$def["solve"]}";
        $result = $this->db->sql_query($sql, $cache_cooldown);
        $row = $this->db->sql_fetchrow($result);
        $this->db->sql_freeresult($result);
        $n_solve = $row["total"];
        ...
        $sql =
            "SELECT COUNT(*) as total FROM " .
            $tbl .
            " WHERE fid={$forum_id} AND status={$def["terminate"]}";
        $result = $this->db->sql_query($sql, $cache_cooldown);
        $row = $this->db->sql_fetchrow($result);
        $this->db->sql_freeresult($result);
        $n_terminate = $row["total"];
        $total =
            ($n_open + $n_dib + $n_fulfill + $n_solve + $n_terminate) / 100;
        if ($total == 0) {
            return false;
        }
    ...
    public function update_request_user_after_topic_deletion($event)
    {
        ...
        $sql =
            "SELECT requester_uid, tid FROM " .
            $tbl["req"] .
            '
            WHERE ' .
            $this->db->sql_in_set("tid", $a_tid);
    ...
    private function generate_topic_list($a_topic_id)
    {
        ...
        $sql =
            "SELECT topic_id, topic_title FROM " .
            TOPICS_TABLE .
            " WHERE $where";
        $result = $this->db->sql_query($sql);
        $rowset = $this->db->sql_fetchrowset($result);
        ...
        foreach ($rowset as $row) {
            $html["body"][] =
                '<tr><td><a target="_blank" href="/viewtopic.php?t=' .
                $row["topic_id"] .
                '">' .
                $row["topic_title"] .
                "</a>";
    ...
    public function reject_user_with_fulfilled($user_id)
    {
        ...
        $grace_period =
            $this->config[$key] == null ? 604800 : (int) $this->config[$key];
        $time = time();
        $def = $this->container->getParameter("jeb.snahp.req")["def"];
        $sql =
            "SELECT tid FROM " .
        ...
        if ($reqdata["b_nolimit"] || $gdata["snp_req_b_nolimit"]) {
            ...
            if (!($n_left > 0)) {
                if ($n_offset > 0) {
                    ...
                    $reset_time_strn = $this->user->format_date(
                        $reqdata["reset_time"]
                    );
