...
class rh_tags_listener implements EventSubscriberInterface
{
    ...
    public function __construct($request, $sauth, $rh_tags_helper)
    {
    ...
    public function get_tags_whitelist($event)
    {
        ...
        $a_groupname = $this->rh_tags_helper->get_groupnames_from_forum_map(
            $forum_id
        );
        $tags = [];
        foreach ($a_groupname as $groupname) {
            if (
                $tmp = $this->rh_tags_helper->get_tagnames_from_group_map(
                    $groupname
                )
            ) {
                sort($tmp);
