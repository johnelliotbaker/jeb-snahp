...
use jeb\snahp\core\parsers\parsedown;

class parsedown_extra extends parsedown
{
    # ~
    ...
    protected function blockHeader($Line)
    {
        ...
        if (
            ...
            $Block["element"]["attributes"] = $this->parseAttributeData(
                $attributeString
            );

            $Block["element"]["handler"]["argument"] = substr(
                $Block["element"]["handler"]["argument"],
                0,
                $matches[0][1]
            );
    ...
    {
        if (!isset($Block["void"])) {
            $Block["element"]["rawHtml"] = $this->processTag(
                $Block["element"]["rawHtml"]
            );
        }
    ...
    protected function inlineFootnoteMarker($Excerpt)
    {
        if (preg_match("/^\[\^(.+?)\]/", $Excerpt["text"], $matches)) {
            ...
            if (!isset($this->DefinitionData["Footnote"][$name]["number"])) {
                $this->DefinitionData["Footnote"][$name][
                    "number"
                ] = ++$this->footnoteCount; # » &
            }
    ...
    protected function buildFootnoteElement()
    {
        ...
        foreach (
            ...
            as $definitionId => $DefinitionData
        ) {
    ...
    protected function processTag($elementMarkup)
    {
        ...
        $elementMarkup = mb_convert_encoding(
            $elementMarkup,
            "HTML-ENTITIES",
            "UTF-8"
        );
        ...
        if ($DOMDocument->documentElement->getAttribute("markdown") === "1") {
            ...
            foreach ($DOMDocument->documentElement->childNodes as $Node) {
                ...
                if (
                    $Node instanceof DOMElement and
