...
class parsedown
{
        ...
        foreach ($lines as $line) {
            if (chop($line) === "") {
                if (isset($CurrentBlock)) {
                    $CurrentBlock["interrupted"] = isset(
    ...
    protected function blockFencedCodeContinue($Line, $Block)
    {
        ...
        if (
            ($len = strspn($Line["text"], $Block["char"])) >=
                $Block["openerLength"] and
            chop(substr($Line["text"], $len), " ") === ""
        ) {
            $Block["element"]["element"]["text"] = substr(
                $Block["element"]["element"]["text"],
                1
            );
    ...
    protected function blockList($Line, array $CurrentBlock = null)
    {
        ...
        if (
            ...
            $Block["data"]["markerTypeRegex"] = preg_quote(
                $Block["data"]["markerType"],
                "/"
            );
            ...
            if ($name === "ol") {
                $listStart =
                    ltrim(
                        strstr($matches[1], $Block["data"]["markerType"], true),
                        "0"
                    ) ?:
                    "0";
                ...
                if ($listStart !== "1") {
                    if (
                        isset($CurrentBlock) and
                        $CurrentBlock["type"] === "Paragraph" and
    ...
    protected function blockRule($Line)
    {
        ...
        if (
            substr_count($Line["text"], $marker) >= 3 and
    ...
    protected function lineElements($text, $nonNestables = [])
    {
        ...
        while ($excerpt = strpbrk($text, $this->inlineMarkerList)) {
            ...
            foreach ($this->InlineTypes[$marker] as $inlineType) {
                ...
                $Inline["element"]["nonNestables"] = isset(
    ...
    protected function handle(array $Element)
    {
        if (isset($Element["handler"])) {
            ...
            $Element[$destination] = $this->{$function}(
                $argument,
                $Element["nonNestables"]
            );
    ...
    protected function elementApplyRecursive($closure, array $Element)
    {
        ...
        if (isset($Element["elements"])) {
            $Element["elements"] = $this->elementsApplyRecursive(
                $closure,
                $Element["elements"]
            );
        } elseif (isset($Element["element"])) {
            $Element["element"] = $this->elementApplyRecursive(
                $closure,
                $Element["element"]
            );
        }
    ...
    {
        if (isset($Element["elements"])) {
            $Element["elements"] = $this->elementsApplyRecursiveDepthFirst(
                $closure,
                $Element["elements"]
            );
        } elseif (isset($Element["element"])) {
            $Element["element"] = $this->elementsApplyRecursiveDepthFirst(
                $closure,
                $Element["element"]
            );
        }
    ...
    protected function elementsApplyRecursiveDepthFirst(
        ...
        foreach ($Elements as &$Element) {
            $Element = $this->elementApplyRecursiveDepthFirst(
                $closure,
                $Element
            );
        }
    ...
    protected function li($lines)
    {
        ...
        if (
            !in_array("", $lines) and
            isset($Elements[0]) and
            isset($Elements[0]["name"]) and
            $Elements[0]["name"] === "p"
        ) {
