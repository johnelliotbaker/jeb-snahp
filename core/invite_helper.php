...
class invite_helper
{
    ...
    public function increase_invite_points($user_id, $amount)
    {
        ...
        if (!$user_data) {
            return $this->insert_invite_users(
                [$user_id],
                $n_available = $amount
            );
        }
    ...
    public function update_invite_users($a_user_id, $data)
    {
        ...
        foreach ($a_user_id as $key => $user_id) {
            ...
            $invite_user_data = $this->select_invite_users(
    ...
    public function insert_invite($inviter_id)
    {
        ...
        if (
            $invite_user_data["n_available"] <= 0 ||
            $invite_user_data["b_ban"]
        ) {
            return false;
        }
        $tbl = $this->phpbb_container->getParameter("jeb.snahp.tables");
    ...
    public function delete_valid_invite_from_user($user_id)
    {
        ...
        $sql =
            "DELETE FROM " .
            $tbl .
            " WHERE inviter_id={$user_id} AND b_active=1";
        $this->db->sql_query($sql);
        return true;
    }
        ...
        foreach ($rowset as $k => $row) {
            if ($b_digest) {
                ...
                $row["create_time"] = $row["create_time"]
                    ? $this->user->format_date($row["create_time"])
                    : "";
                $row["expiration_time"] = $row["expiration_time"]
                    ? $this->user->format_date($row["expiration_time"])
                    : "";
            }
            $row["redeem_time"] = $row["redeem_time"]
                ? $this->user->format_date($row["redeem_time"])
                : "";
            $row["status_strn"] = $row["b_active"]
                ? '<span style="color:#0B0"><b>VALID</b></span>'
                : '<span style="color:#B00"><b>VOID</b></span>';
