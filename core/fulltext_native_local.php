 ...
 */
class fulltext_native_local extends \phpbb\search\base
{
    ...
    public function __construct(
        ...
        $this->word_length = [
            "min" => (int) $this->config["fulltext_native_min_chars"],
            "max" => (int) $this->config["fulltext_native_max_chars"],
        ];
    ...
    public function split_keywords($keywords, $terms)
    {
        ...
        if (count($exact_words)) {
            $sql =
                ...
                '
				WHERE ' .
                $this->db->sql_in_set("word_text", $exact_words) .
                '
        ...
        foreach ($query as $word) {
            ...
            if (is_array($word)) {
                ...
                foreach ($word as $i => $word_part) {
                    if (strpos($word_part, "*") !== false) {
                        $id_words[] =
                            '\'' .
                            $this->db->sql_escape(
                                str_replace("*", "%", $word_part)
                            ) .
                            '\'';
                        $non_common_words[] = $word_part;
                    } elseif (isset($words[$word_part])) {
                        $id_words[] = $words[$word_part];
                        $non_common_words[] = $word_part;
                    } else {
                        $len = utf8_strlen($word_part);
                        if (
                            $len < $this->word_length["min"] ||
                ...
                elseif (!$ignore_no_id && count($non_common_words)) {
                    trigger_error(
                        sprintf(
                            $this->user->lang["WORDS_IN_NO_POST"],
                            implode(
                                $this->user->lang["COMMA_SEPARATOR"],
                                $non_common_words
                            )
                        )
                    );
    ...
    public function keyword_search(
        ...
        if (count($this->must_not_contain_ids)) {
            $sql_array["LEFT_JOIN"][] = [
                ...
                "ON" =>
                    ...
                    ($title_match ? " AND m$m_num.$title_match" : "") .
                    " AND m$m_num.post_id = m0.post_id",
            ];
        ...
        foreach ($this->must_exclude_one_ids as $ids) {
            ...
            foreach ($ids as $id) {
                ...
                $sql_array["LEFT_JOIN"][] = [
                    "FROM" => [SEARCH_WORDMATCH_TABLE => "m" . $m_num],
                    "ON" =>
                        "m$m_num.word_id = $id AND m$m_num.post_id = m0.post_id" .
                        ($title_match ? " AND m$m_num.$title_match" : ""),
                ];
        ...
        if (count($author_ary)) {
            if ($author_name) {
                ...
                $sql_author =
                    ...
                    $this->db->sql_in_set(
                        "p.poster_id",
                        array_diff($author_ary, [ANONYMOUS]),
                        false,
                        true
                    ) .
                    " OR p.post_username " .
        ...
        if (!$total_results) {
            ...
            switch ($this->db->get_sql_layer()) {
                ...
                case "sqlite3":
                    $sql_array_count["SELECT"] =
                        $type == "posts"
                            ? "DISTINCT p.post_id"
                            : "DISTINCT p.topic_id";
                    $sql =
                        "SELECT COUNT(" .
                        ($type == "posts" ? "post_id" : "topic_id") .
                        ') as total_results
							FROM (' .
                ...
                default:
                    $sql_array_count["SELECT"] =
                        $type == "posts"
                            ? "COUNT(DISTINCT p.post_id) AS total_results"
                            : "COUNT(DISTINCT p.topic_id) AS total_results";
                    $sql = !$sql
                        ? $this->db->sql_build_query("SELECT", $sql_array_count)
                        : $sql;

                    $result = $this->db->sql_query($sql);
                    $total_results = (int) $this->db->sql_fetchfield(
                        "total_results"
                    );
                    $this->db->sql_freeresult($result);
        ...
        $sql_sort =
            $sort_by_sql[$sort_key] . ($sort_dir == "a" ? " ASC" : " DESC");
        ...
        if (!$total_results && $is_mysql) {
            ...
            $sql_array["SELECT"] =
                "SQL_CALC_FOUND_ROWS " . $sql_array["SELECT"];
        }

        $sql_array["WHERE"] = implode(" AND ", $sql_where);
        $sql_array["GROUP_BY"] = $group_by
            ? ($type == "posts" ? "p.post_id" : "p.topic_id") .
                ", " .
        ...
        if ($start >= $total_results) {
            ...
            while ($row = $this->db->sql_fetchrow($result)) {
                $id_ary[] =
                    (int) $row[$type == "posts" ? "post_id" : "topic_id"];
            }
            $this->db->sql_freeresult($result);
        }
    ...
    public function author_search(
        ...
        $sql_fora = count($ex_fid_ary)
            ? " AND " . $this->db->sql_in_set("p.forum_id", $ex_fid_ary, true)
            : "";
        $sql_time = $sort_days
            ? " AND p.post_time >= " . (time() - $sort_days * 86400)
            : "";
        $sql_topic_id = $topic_id ? " AND p.topic_id = " . (int) $topic_id : "";
        $sql_firstpost = $firstpost_only
        ...
        if (!$total_results) {
            switch ($this->db->get_sql_layer()) {
                ...
                default:
                    if ($type == "posts") {
                        $sql =
                            'SELECT COUNT(p.post_id) as total_results
							FROM ' .
                            POSTS_TABLE .
                            " p" .
                            ($firstpost_only
                        ...
                        if ($this->db->get_sql_layer() == "sqlite3") {
                            ...
                            $sql =
                                "SELECT COUNT(DISTINCT t.topic_id) as total_results";
                        }
    ...
    public function split_message($text, $b_subject = false)
    {
        ...
        // Do not index code
        $match[] =
            "#\[code(?:=.*?)?(\:?[0-9a-z]{5,})\].*?\[\/code(\:?[0-9a-z]{5,})\]#is";
        // BBcode
        $match[] =
            "#\[\/?[a-z0-9\*\+\-]+(?:=.*?)?(?::[a-z])?(\:?[0-9a-z]{5,})\]#";
         ...
         */
        $word = strtok(
            $this->cleanup(preg_replace($match, " ", strip_tags($text)), -1),
            " "
        );
        ...
        while (strlen($word)) {
            ...
            if ($len < $min) {
                 ...
                 */
                if (
                    (strncmp($word, self::UTF8_HANGUL_FIRST, 3) < 0 ||
                        strncmp($word, self::UTF8_HANGUL_LAST, 3) > 0) &&
                    (strncmp($word, self::UTF8_CJK_FIRST, 3) < 0 ||
                        strncmp($word, self::UTF8_CJK_LAST, 3) > 0) &&
                    (strncmp($word, self::UTF8_CJK_B_FIRST, 4) < 0 ||
                        strncmp($word, self::UTF8_CJK_B_LAST, 4) > 0)
                ) {
                    $word = strtok(" ");
                    continue;
                }
    ...
    public function index(
        ...
        if ($mode == "edit") {
            ...
            $words["add"]["post"] = array_diff(
                $split_text,
                array_keys($cur_words["post"])
            );
            $words["add"]["title"] = array_diff(
                $split_title,
                array_keys($cur_words["title"])
            );
            $words["del"]["post"] = array_diff(
                array_keys($cur_words["post"]),
                $split_text
            );
            $words["del"]["title"] = array_diff(
                array_keys($cur_words["title"]),
                $split_title
            );
        } else {
        ...
        $unique_add_words = array_unique(
            array_merge($words["add"]["post"], $words["add"]["title"])
        );
        ...
        if (count($unique_add_words)) {
            $sql =
                ...
                '
				WHERE ' .
                $this->db->sql_in_set("word_text", $unique_add_words);
            $result = $this->db->sql_query($sql);
        ...
        foreach ($words["del"] as $word_in => $word_ary) {
            ...
            if (count($word_ary)) {
                ...
                $sql =
                    ...
                    '
					WHERE ' .
                    $this->db->sql_in_set("word_id", $sql_in) .
                    '
						AND post_id = ' .
                    intval($post_id) .
                    "
        ...
        foreach ($words["add"] as $word_in => $word_ary) {
            ...
            if (count($word_ary)) {
                $sql =
                    ...
                    '
					WHERE ' .
                    $this->db->sql_in_set("word_text", $word_ary);
                $this->db->sql_query($sql);
    ...
    public function index_remove($post_ids, $author_ids, $forum_ids)
    {
        if (count($post_ids)) {
            $sql =
                ...
                ' w
				WHERE ' .
                $this->db->sql_in_set("m.post_id", $post_ids) .
                '
					AND w.word_id = m.word_id';
            $result = $this->db->sql_query($sql);
            ...
            if (count($title_word_ids)) {
                $sql =
					...
					SET word_count = word_count - 1
					WHERE ' .
                    $this->db->sql_in_set("word_id", $title_word_ids) .
                    '
						AND word_count > 0';
                $this->db->sql_query($sql);
            }
            ...
            if (count($message_word_ids)) {
                $sql =
					...
					SET word_count = word_count - 1
					WHERE ' .
                    $this->db->sql_in_set("word_id", $message_word_ids) .
                    '
						AND word_count > 0';
                $this->db->sql_query($sql);
            }
            ...
            $sql =
                ...
                '
				WHERE ' .
                $this->db->sql_in_set("post_id", $post_ids);
            $this->db->sql_query($sql);
        }

        $this->destroy_cache(
            array_unique($word_texts),
            array_unique($author_ids)
        );
    }
    ...
    public function tidy()
    {
        ...
        // Remove common words
        if (
            $this->config["num_posts"] >= 100 &&
            $this->config["fulltext_native_common_thres"]
        ) {
            $common_threshold =
                ((float) $this->config["fulltext_native_common_thres"]) / 100.0;
            ...
            $sql =
                ...
                '
				WHERE word_count > ' .
                floor($this->config["num_posts"] * $common_threshold) .
                '
					OR word_common = 1';
            $result = $this->db->sql_query($sql);
    ...
    public function delete_index($acp_module, $u_action)
    {
        switch ($this->db->get_sql_layer()) {
            ...
            default:
                ...
                $this->db->sql_query(
                    "TRUNCATE TABLE " . SEARCH_WORDMATCH_TABLE
                );
    ...
    {
        $this->stats["total_words"] = $this->db->get_estimated_row_count(
            SEARCH_WORDLIST_TABLE
        );
        $this->stats["total_matches"] = $this->db->get_estimated_row_count(
            SEARCH_WORDMATCH_TABLE
        );
    }
    ...
    protected function cleanup(
            ...
            do {
                ...
                if ($c < "\x80") {
                    ...
                    if (is_int($sb_pos)) {
                         ...
                         */
                        $sb_match =
                            substr($sb_match, 0, $sb_pos) .
                            substr($sb_match, $sb_pos + 1);
                        $sb_replace =
                            substr($sb_replace, 0, $sb_pos) .
                            substr($sb_replace, $sb_pos + 1);
                        $legal_ascii .= $c;
                    }
        ...
        do {
            ...
            if (
                ($utf_char >= self::UTF8_HANGUL_FIRST &&
                    $utf_char <= self::UTF8_HANGUL_LAST) ||
                ($utf_char >= self::UTF8_CJK_FIRST &&
                    $utf_char <= self::UTF8_CJK_LAST) ||
                ($utf_char >= self::UTF8_CJK_B_FIRST &&
                    $utf_char <= self::UTF8_CJK_B_LAST)
            ) {
                /**
            ...
            if (isset($utf_char[2])) {
                if (isset($utf_char[3])) {
                     ...
                     */
                    $idx =
                        ((ord($utf_char[0]) & 0x07) << 7) |
                        ((ord($utf_char[1]) & 0x3f) << 1) |
                        ((ord($utf_char[2]) & 0x20) >> 5);
                } else {
                    /**
                     ...
                     */
                    $idx =
                        ((ord($utf_char[0]) & 0x07) << 1) |
                        ((ord($utf_char[1]) & 0x20) >> 5);
                }
            } else {
                /**
                 * 110x xxxx 10xx xxxx
            ...
            if (!isset($conv_loaded[$idx])) {
                ...
                $file =
                    ...
                    "includes/utf/data/search_indexer_" .
                    $idx .
                    "." .
                    $this->php_ext;
    ...
    public function acp()
    {
        ...
        $tpl =
            '
		<dl>
			<dt><label for="fulltext_native_load_upd">' .
            $this->user->lang["YES_SEARCH_UPDATE"] .
            $this->user->lang["COLON"] .
            "</label><br /><span>" .
            $this->user->lang["YES_SEARCH_UPDATE_EXPLAIN"] .
            '</span></dt>
			<dd><label><input type="radio" id="fulltext_native_load_upd" name="config[fulltext_native_load_upd]" value="1"' .
            ($this->config["fulltext_native_load_upd"]
                ? ' checked="checked"'
                : "") .
            ' class="radio" /> ' .
            $this->user->lang["YES"] .
            '</label><label><input type="radio" name="config[fulltext_native_load_upd]" value="0"' .
            (!$this->config["fulltext_native_load_upd"]
                ? ' checked="checked"'
                : "") .
            ' class="radio" /> ' .
		...
		<dl>
			<dt><label for="fulltext_native_min_chars">' .
            $this->user->lang["MIN_SEARCH_CHARS"] .
            $this->user->lang["COLON"] .
            "</label><br /><span>" .
            $this->user->lang["MIN_SEARCH_CHARS_EXPLAIN"] .
            '</span></dt>
			<dd><input id="fulltext_native_min_chars" type="number" min="0" max="255" name="config[fulltext_native_min_chars]" value="' .
            (int) $this->config["fulltext_native_min_chars"] .
            '" /></dd>
		</dl>
		<dl>
			<dt><label for="fulltext_native_max_chars">' .
            $this->user->lang["MAX_SEARCH_CHARS"] .
            $this->user->lang["COLON"] .
            "</label><br /><span>" .
            $this->user->lang["MAX_SEARCH_CHARS_EXPLAIN"] .
            '</span></dt>
			<dd><input id="fulltext_native_max_chars" type="number" min="0" max="255" name="config[fulltext_native_max_chars]" value="' .
            (int) $this->config["fulltext_native_max_chars"] .
            '" /></dd>
		</dl>
		<dl>
			<dt><label for="fulltext_native_common_thres">' .
            $this->user->lang["COMMON_WORD_THRESHOLD"] .
            $this->user->lang["COLON"] .
            "</label><br /><span>" .
            $this->user->lang["COMMON_WORD_THRESHOLD_EXPLAIN"] .
            '</span></dt>
			<dd><input id="fulltext_native_common_thres" type="text" name="config[fulltext_native_common_thres]" value="' .
            (float) $this->config["fulltext_native_common_thres"] .
            '" /> %</dd>
		</dl>
		';
