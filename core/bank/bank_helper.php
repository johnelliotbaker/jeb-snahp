...
class bank_helper
{
    ...
    public function append_exchange_to_transaction(
        $amount,
        $transaction_id,
        $comment = ""
    ) {
    ...
    private function withdraw_token_from_user($user_id, $amount)
    {
        ...
        $sql =
            "Update " .
            USERS_TABLE .
            " SET ${deposit_strn} WHERE user_id=${user_id}";
    ...
    public function perform_exchange($exchange_rate_id, $direction, $amount)
    {
        ...
        if ($direction == "buy") {
            ...
            if ($exchange_rate["type"] == "invitation_points") {
                [$b, $status] = $this->exchange_invitation_points_for_tokens(
                    $amount,
                    $user_id,
                    $exchange_rate
                );
    ...
    private function exchange_invitation_points_for_tokens(
        $amount,
        $user_id,
        $exchange_rate
    ) {
    ...
    {
        [$b, $status] = $this->perform_exchange(
            $type_name,
            $direction,
            $amount
        );
    ...
    public function get_transaction_history($user_id)
    {
        ...
        foreach ($rowset as &$row) {
            $row["created_time_strn"] = $this->user->format_date(
                $row["created_time"]
            );
        }
        return $rowset;
