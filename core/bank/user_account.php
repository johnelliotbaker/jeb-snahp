...
class user_account
{
    ...
    public function get_balance($user_id)
    {
        $user_id = (int) $user_id;
        $sql =
            "SELECT snp_bank_n_balance FROM " .
            USERS_TABLE .
    ...
    public function set_balance($user_id, $value, $broker_id = -1)
    {
        ...
        $b_log = $this->bank_transaction_logger->create_single_transaction(
            $user_id,
            $broker_id,
            $data
        );
    ...
    public function create_transaction_and_deposit(
        $amount,
        $user_id,
        $broker_id = -1,
        $comment = ""
    ...
    public function create_transaction_and_withdraw(
        $amount,
        $user_id,
        $broker_id = -1,
        $comment = ""
    ...
    public function do_exchange(
        ...
        $amount = (int) $amount;
        $exchange_rate = $this->exchange_rates->get_exchange_rate(
            $exchange_rate_id
        );
        if (!$exchange_rate) {
        ...
        if ($direction == "buy") {
            ...
            if ($exchange_rate["type"] == "invitation_points") {
                [$b, $status] = $this->sell_invitation_points(
                    $amount,
                    $user_id,
                    $exchange_rate
                );
    ...
    private function sell_invitation_points($amount, $user_id, $exchange_rate)
    {
        ...
        $fund =
            $invite_user_data && $invite_user_data["n_available"]
                ? $invite_user_data["n_available"]
                : 0;
        if (!$invite_user_data || $invite_user_data["n_available"] < $amount) {
            return [
                0,
                "You are trying to sell ${amount} invitation points but you only have ${fund} available.",
            ];
        }
    ...
    public function giveInvitationPoints(
        $amount,
        $user_id,
        $type = "moderation",
        $comment = ""
        ...
        $this->bank_transaction_logger->create_single_transaction(
            $user_id,
            $broker_id,
            $data
        );
