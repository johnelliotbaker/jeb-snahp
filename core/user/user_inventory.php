...
class user_inventory
{
    ...
    public function get_inventory_by_product_class(
        $product_class_id,
        $user_id = null
    ) {
        $user_id = $user_id === null ? $this->user_id : (int) $user_id;
    ...
    {
        $row = $this->get_inventory_by_product_class(
            $product_class_id,
            $user_id
        );
        if ($row) {
    ...
    public function do_add_item_with_logging(
        $product_class_id,
        $quantity,
        $user_id = null,
        $comment = ""
    ) {
        $b_item_added = $this->do_add_item(
            $product_class_id,
            $quantity,
            $user_id
        );
        $product_class = $this->product_class->get_product_class(
            $product_class_id
        );
        ...
        $this->market_transaction_logger->create_single_item_invoice(
            $user_id,
            $broker_id = -1,
            $data
        );
    ...
    public function doRemoveItemWithLogging(
        $productClassId,
        $quantity,
        $userId,
        $comment = ""
        ...
        $productClass = $this->product_class->get_product_class(
            $productClassId
        );
    ...
    public function doRemoveItem($productClassId, $quantity, $userId)
    {
        ...
        if ($quantity < 0) {
            trigger_error(
                "Quantity cannot be less than 0. Error Code: 72b59fc267"
            );
        }
    ...
    public function set_item_quantity_public(
        $product_class_id,
        $quantity,
        $user_id,
        $broker_id = -1
        ...
        $b_success = false;
        if ($quantity < 1) {
            $b_success = $this->delete_item(
                $product_class_id,
                $user_id,
                $broker_id
            );
        } else {
            $b_success = $this->upsert_inventory(
                $product_class_id,
                $quantity,
                $user_id,
                $broker_id
    ...
    private function upsert_inventory(
        $product_class_id,
        $quantity,
        $user_id,
        $broker_id = -1
    ...
    public function log_moderation(
        $product_class_id,
        $quantity,
        $user_id,
        $broker_id,
    ...
    private function log($user_id, $broker_id, $data)
    {
        ...
        return $this->market_transaction_logger->create_single_item_invoice(
            $user_id,
            $broker_id,
            $data
        );
