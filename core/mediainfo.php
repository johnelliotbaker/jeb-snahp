...
class mediainfo
{
    ...
    private function generate_general_content($data, $extra = [])
    {
        ...
        foreach ($a_element as $element) {
            $entry["type"] = "kv";
            $entry["content"] = $this->{$function_prefix . $element["f"]}(
                $data
            );
    ...
    private function generate_subtitle_content($data, $extra = [])
    {
        ...
        foreach ($a_data as $data) {
            $tmp = ["id" => $i];
            foreach ($a_element as $element) {
                $tmp[$element["alias"]] = $this->{$function_prefix .
                    $element["f"]}($data);
            }
            $entry["type"] = "fullwidth";
            $language = $this->get_language_image_or_strn($tmp["Language"], [
                "append_text" => true,
            ])["value"];
            $format = $tmp["Format"];
            $html =
                '<div class="row"><div class="col-auto subtitle_key">#' .
                $i .
                ':</div><div class="float-right subtitle_value col">' .
                $language .
                " ${format}</div></div>";
            $entry["content"] = $html;
    ...
    private function get_audio_channels($data)
    {
        $b = preg_match(
            "#(\d+).*#is",
            $this->get_val_or_null("Channel(s)", $data),
            $match
        );
        if (!$b) {
            return "";
    ...
    private function get_language_image_or_strn($strn, $options = [])
    {
        ...
        $img_html =
            '<img class="flag" src="/ext/jeb/snahp/styles/all/template/flags/4x3/' .
            $country_code .
            '.svg" title="' .
            $strn .
            '"></img>';
    ...
    private function generate_audio_content($data, $extra = [])
    {
        ...
        foreach ($a_data as $data) {
            ...
            $html =
                '<div class="row"><div class="col-auto audio_key">#' .
                $i .
                ':</div><div class="float-right audio_value col">' .
                $value .
                "</div></div>";
            $entry["type"] = "fullwidth";
            $entry["content"] = $html;
    ...
    private function validate_data($data)
    {
        ...
        if (
            !(
                array_key_exists("Video", $this->data) ||
                array_key_exists("Video #1", $this->data)
            )
        ) {
            return false;
        }
        if (
            !(
                array_key_exists("Audio", $this->data) ||
                array_key_exists("Audio #1", $this->data)
            )
        ) {
            return false;
        }
    ...
    public function make_mediainfo($strn)
    {
        ...
        $res[] = "";
        $res[] =
            '<div class="twbs mediainfo"><div class="container-fluid"><div class="row">';
        $res[] = '<div class="col-12 col-md-4 general">';
        $res[] = '<div class="col-12 title">General</div>';
        $res[] = $this->make_bucket(
            $this->generate_general_content($this->data, $subtitle),
            ["css_prefix" => "general_"]
        );
        $res[] = "</div>";
        ...
        $res = join("", $res);
        $res .=
            '<div class="codebox" style="margin-top:0px; box-shadow: none; margin-left: 0px; margin-right: 0px;"><p style="border-bottom: none;">Code: <a href="#" onclick="selectCode(this); return false;">Select all</a></p><pre><code>' .
            $original .
            "</code></pre></div>";
        return $res;
    }
    ...
    private function make_collapsable($a_data, $title = "")
    {
        ...
        $html =
            '<div class="row collapse_handle pointer noselect" data-toggle="collapse" href="#' .
            $uuid .
            '" role="button" aria-expanded="false" aria-controls="' .
            $uuid .
            '"><i class="icon fa-plus-square-o collapse_plus" aria-hidden="true"></i><div class="ml-1">' .
            $title .
            '</div></div>
      <div class="row"> <div class="col"> <div class="collapse" id="' .
            $uuid .
            '">
      <div class="card card-body collapsable">' .
            $html .
            '</div>
      </div></div></div>';
        $entry["type"] = "fullwidth";
    ...
    private function auto_convert_to_html($a_data, $cfg = [])
    {
        ...
        foreach ($a_data as $k => $v) {
            ...
            switch ($type) {
                case "fullwidth":
                    $res[] = $this->convert_to_fullwidth_html(
                        $v["content"],
                        $cfg
                    );
                    break;
                case "kv":
                    $res[$k] = $this->convert_to_kv_html(
                        $k,
                        $v["content"],
                        $cfg
                    );
    ...
    {
        $css_prefix = array_key_exists("css_prefix", $cfg)
            ? $cfg["css_prefix"]
            : "";
