...
class helper
{
    ...
    public function __construct(
        ...
        $this->event_def = $container->getParameter("jeb.snahp.giftbox")[
            $this->event_name
        ];
    ...
    private function time_to_next_interval(
        $time,
        $start_time,
        $end_time,
        $latest_unwrap_time
        ...
        for ($i = 0; $i < count($range) - 1; $i++) {
            ...
            if (
                $time >= $s &&
                $time < $e &&
                $latest_unwrap_time >= $s &&
                $latest_unwrap_time < $e
            ) {
                return [false, $e - $time];
            }
        }
    ...
    public function get_unwrap_status($user_id)
    {
        ...
        [$ready, $time_left] = $this->time_to_next_interval(
            $time,
            $start_time,
            $end_time,
            $created_time
    ...
    public function manual_giveaway($simulate = true)
    {
        ...
        if ($simulate) {
            ...
            $this->send_message([
                "0" =>
                    "This is simulation: No actual reward given. Notification sent.",
            ]);
        }
        foreach ($rowset as $row) {
            ...
            if ($simulate) {
                $this->send_message([
                    "Simulating: user: ${user_id} with count: ${count}",
                ]);
            }
            if ($i_loop % 10 == 0) {
            ...
            if ($count >= 5 && !$b5) {
                if ($simulate) {
                    $this->send_message([
                        "0" => "Giving user_id: ${user_id} completion 5 - $5000",
                    ]);
                } else {
                    $item_def = $item_def5;
            ...
            if ($count >= 9 && !$b9) {
                if ($simulate) {
                    $this->send_message([
                        "0" => "Giving user_id: ${user_id} completion 9 - $5000",
                    ]);
                } else {
                    $item_def = $item_def9;
                    $this->insert_item($item_def["type"], $user_id, $extra);
            ...
            if ($count >= 10 && !$b10) {
                if ($simulate) {
                    $this->send_message([
                        "0" => "Giving user_id: ${user_id} completion 10 - $10000",
                    ]);
                } else {
                    $item_def = $item_def10;
                    $this->insert_item($item_def["type"], $user_id, $extra);
                ...
                $notification_data["description"] =
                    '10 Days Completion Bonus ($10,000)';
                $notification->add_notifications(
                    "jeb.snahp.notification.type.simple",
                    $notification_data
                );
    ...
    {
        $user_id =
            $user_id === null ? ($user_id = $this->user_id) : (int) $user_id;
        $sql =
    ...
    {
        $this->bank_user_account->create_transaction_and_deposit(
            $amount,
            $user_id,
            -1,
            $comment
