...
class ThanksUsers
{
    ...
    protected $sauth;
    private $data;
    public function __construct($db, $user, $config, $template, $tbl, $sauth)
    {
        $this->db = $db;
        $this->user = $user;
        $this->config = $config;
    ...
    {
        return $this->sauth->getMaxFromGroupMemberships(
            $userId,
            "tfp_n_per_cycle"
        );
    }
    ...
    private function createThanksUser($userId)
    {
        ...
        $sql =
            "INSERT INTO " . $tbl . $this->db->sql_build_array("INSERT", $data);
    ...
    public function rejectExcessiveThanksPerCycle($thanksUserData)
    {
        ...
        if ($this->nAllowedPerCycle < 1) {
            trigger_error(
                "You cannot give any more thanks. Error Code: 432e131525"
            );
        }
        $curr = time();
