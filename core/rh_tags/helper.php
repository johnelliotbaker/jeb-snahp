...
class helper
{
    ...
    public function get_tagnames_from_group_map($groupname, $cd=30)
    {
        ...
        return array_map(function($arg){return $arg['tagname'];}, $rowset);
    }
    ...
    public function get_groupnames_from_forum_map($forum_id, $cd=30)
    {
        ...
        $rowset = $this->db->sql_fetchrowset($result);
        $this->db->sql_freeresult($result);
        return array_map(function($arg){return $arg['groupname'];}, $rowset);
    }

    public function insert_or_update_group_map($data)
    {
        $sql = 'INSERT INTO ' . $this->tbl['tags_group_map'] . $this->db->sql_build_array('INSERT', $data) . '
    ...
    {
        $sql = 'INSERT INTO ' . $this->tbl['tags_forum_map'] . $this->db->sql_build_array('INSERT', $data) . '
            ON DUPLICATE KEY UPDATE ' . $this->db->sql_build_array('UPDATE', $data);
        $this->db->sql_query($sql);
    }
    ...
    {
        $forum_id = (int) $forum_id;
        $sql = 'DELETE FROM ' . $this->tbl['tags_forum_map'] . " WHERE forum_id=${forum_id}";
        $this->db->sql_query($sql);
    }
    ...
    public function delete_group_map($groupname, $tagnames)
    {
        ...
        $sql = 'DELETE FROM ' . $this->tbl['tags_group_map'] . " WHERE groupname='${groupname}' AND $sql_in_set";
        $this->db->sql_query($sql);
    }
