...
class mysqli extends \phpbb\db\driver\mysql_base
{
    ...
    public function sql_connect(
        ...
        $this->server = $this->persistency
            ? "p:" . ($sqlserver ? $sqlserver : "localhost")
            : $sqlserver;
    ...
    public function sql_server_info($raw = false, $use_cache = true)
    {
        ...
        if (
            !$use_cache ||
            empty($cache) ||
            ($this->sql_server_version = $cache->get("mysqli_version")) ===
                false
        ) {
            $result = @mysqli_query(
                $this->db_connect_id,
        ...
        return $raw
            ...
            : "MySQL(i) " . $this->sql_server_version;
    }
    ...
    public function sql_query($query = "", $cache_ttl = 0)
    {
        if ($query != "") {
            ...
            if ($this->query_result === false) {
                if (
                    ($this->query_result = @mysqli_query(
                        $this->db_connect_id,
                        $query
                    )) === false
                ) {
    ...
    {
        return $this->db_connect_id
            ? @mysqli_affected_rows($this->db_connect_id)
            : false;
    }
    ...
    {
        return $this->db_connect_id
            ? @mysqli_insert_id($this->db_connect_id)
            : false;
    }
    ...
    public function _sql_report($mode, $query = "")
    {
        ...
        if ($test_prof === null) {
            ...
            if (
                strpos(
                    mysqli_get_server_info($this->db_connect_id),
                    "community"
                ) !== false
            ) {
