...
class digg extends \phpbb\notification\type\base
{
	...
	public function get_url()
	{
		...
		return append_sid($this->phpbb_root_path . 'viewtopic.' . $this->php_ext, "t={$topic_id}");
	}
    ...
    public function get_title()
    {
        ...
        $username = $this->user_loader->get_username($this->get_data('topic_poster'), 'no_profile');
	...
	public function find_users_for_notification($data, $options = array())
	{
        ...
        $sql = 'SELECT user_id
            FROM ' . $tbl['digg_slave'] . '
            WHERE ' . $this->db->sql_in_set('topic_id', $data['topic_id']) . '
            AND user_id <> ' . (int) $data['topic_poster'];
    ...
    public function create_insert_array($data, $pre_create_data = array())
	{
        ...
        $this->set_data('topic_first_poster_name', $data['topic_first_poster_name']);
