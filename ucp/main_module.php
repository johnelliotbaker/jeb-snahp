...
class main_module
{
    ...
    private function is_custom_rank_purchased($user_id)
    {
        ...
        $sql =
            "SELECT * FROM " .
            $tbl["mrkt_product_classes"] .
            " WHERE name='custom_rank'";
        ...
        $sql =
            "SELECT * FROM " .
            $tbl["user_inventory"] .
            " WHERE user_id=${user_id} AND product_class_id=${pcid}";
        $result = $db->sql_query($sql);
        $row = $db->sql_fetchrow($result);
        $db->sql_freeresult($result);
        if (!$row) {
    ...
    private function get_quick_search_upgrade_count($user_id)
    {
        ...
        $sql =
            "SELECT * FROM " .
            $tbl["mrkt_product_classes"] .
            " WHERE name='search_cooldown_reducer'";
        $result = $db->sql_query($sql);
        $row = $db->sql_fetchrow($result);
        $db->sql_freeresult($result);
        $pcid = $row["id"];
    ...
    public function handle_block($cfg)
    {
        global $phpbb_container,
            $user,
            $auth,
    ...
    public function handle_custom($cfg)
    {
        global $phpbb_container,
            $user,
            $auth,
        ...
        $b_enable = true;
        $search_interval = $sauth->getMinFromGroupMemberships(
            $user_id,
            "snp_search_interval"
        );
    ...
    public function handle_invite($cfg)
    {
        ...
        $ih = new \jeb\snahp\core\invite_helper(
            $phpbb_container,
            $user,
            $auth,
            $request,
        ...
        $invite_users_data = $ih->select_invite_users(
        ...
        $invite_data = $ih->get_invite_list(
            $user_id,
            $b_digest = true,
            $start = $start,
            $limit = $per_page
    ...
    public function handle_visibility($cfg)
    {
        global $db, $request, $template, $user;
        $user_id = $user->data["user_id"];
        ...
        $data = [
            "snp_disable_avatar_thanks_link" => $request->variable(
                "snp_disable_avatar_thanks_link",
                $user->data["snp_disable_avatar_thanks_link"]
            ),
            "snp_thanks_b_topic" => $request->variable(
                "snp_thanks_b_topic",
                $user->data["snp_thanks_b_topic"]
            ),
            "snp_enable_at_notify" => $request->variable(
                "snp_enable_at_notify",
                $user->data["snp_enable_at_notify"]
            ),
            "snp_rep_b_avatar" => $request->variable(
                "snp_rep_b_avatar",
                $user->data["snp_rep_b_avatar"]
            ),
            "snp_rep_b_profile" => $request->variable(
                "snp_rep_b_profile",
                $user->data["snp_rep_b_profile"]
            ),
            "snp_achi_b_avatar" => $request->variable(
                "snp_achi_b_avatar",
                $user->data["snp_achi_b_avatar"]
            ),
            "snp_achi_b_profile" => $request->variable(
                "snp_achi_b_profile",
                $user->data["snp_achi_b_profile"]
            ),
        ];
        ...
        $template_vars = [
            "S_SNP_DISABLE_AVATAR_THANKS_LINK" =>
                $data["snp_disable_avatar_thanks_link"],
    ...
    public function user_belongs_to_groupset($user_id, $groupset_name)
    {
        ...
        if ($this->is_dev_server()) {
            $groupset = $phpbb_container->getParameter("jeb.snahp.groups")[
                "dev"
            ]["set"];
        } else {
            $groupset = $phpbb_container->getParameter("jeb.snahp.groups")[
