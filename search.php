...
$search_forum = $request->variable("fid", [0]);
...
$doc_vars = ["interval"];
extract(
    $phpbb_dispatcher->trigger_event(
...
if (
    ...
    if ($user->data["user_last_search"] > time() - $interval) {
        ...
        trigger_error(
            $user->lang(
                "NO_SEARCH_TIME",
                (int) ($user->data["user_last_search"] + $interval - time())
            )
        );
    }
}
 ...
 * @since 3.1.10-RC1
 */
$vars = ["keywords", "author", "author_id", "search_id", "submit"];
extract(
    $phpbb_dispatcher->trigger_event(
...
if ($keywords || $author || $author_id || $search_id || $submit) {
    ...
    if ($author_id) {
        ...
        if (
            strpos($author, "*") !== false &&
            utf8_strlen(str_replace(["*", "%"], "", $author)) <
                $config["min_search_author_chars"]
        ) {
            trigger_error(
                $user->lang(
                    "TOO_FEW_AUTHOR_CHARS",
                    (int) $config["min_search_author_chars"]
                )
            );
        }
                ...
                ? " username_clean " .
                    $db->sql_like_expression(
                        str_replace(
                            "*",
                            $db->get_any_char(),
                            utf8_clean_string($author)
                        )
                    )
                : " username_clean = '" .
                    $db->sql_escape(utf8_clean_string($author)) .
                    "'";
    ...
    if ($add_keywords) {
        if ($search_terms == "all") {
            ...
            $keywords =
                implode(
                    " |",
                    explode(" ", preg_replace("#\s+#u", " ", $keywords))
                ) .
                " " .
                $add_keywords;
        }
    ...
    $not_in_fid = count($ex_fid_ary)
        ? "WHERE " .
            $db->sql_in_set("f.forum_id", $ex_fid_ary, true) .
            " OR (f.forum_password <> '' AND fa.user_id <> " .
    ...
    while ($row = $db->sql_fetchrow($result)) {
        ...
        if (count($search_forum)) {
            if ($search_child) {
                if (
                    in_array($row["forum_id"], $search_forum) &&
    ...
    $m_approve_posts_fid_sql = $phpbb_content_visibility->get_global_visibility_sql(
        "post",
        $ex_fid_ary,
        "p."
    );
    $m_approve_topics_fid_sql = $phpbb_content_visibility->get_global_visibility_sql(
        "topic",
        $ex_fid_ary,
        "t."
    );
    ...
    if ($keywords) {
        ...
        if (
            ...
            $ignored = count($common_words)
                ? sprintf(
                        $user->lang["IGNORED_TERMS_EXPLAIN"],
                        implode(" ", $common_words)
                    ) . "<br />"
                : "";
                ...
                trigger_error(
                    $ignored .
                        $user->lang(
                            ...
                            $user->lang("CHARACTERS", (int) $word_length["max"])
                        )
                );
            } else {
    ...
    if (!$keywords && count($author_id_ary)) {
        ...
        $show_results = $topic_id
            ? "posts"
            : $request->variable(
                "sr",
                $search_id == "egosearch" ? "topics" : "posts"
            );
        $show_results = $show_results == "posts" ? "posts" : "topics";
    }
    ...
    $sort_by_sql = [
        "a" => "u.username_clean",
        "t" =>
            $show_results == "posts" ? "p.post_time" : "t.topic_last_post_time",
        "f" => "f.forum_id",
        "i" => "t.topic_title",
    ...
    if ($search_id) {
        switch ($search_id) {
            ...
            case "active_topics":
                ...
                $last_post_time_sql = $sort_days
                    ? " AND t.topic_last_post_time > " .
                        (time() - $sort_days * 24 * 3600)
                    : "";

                $sql =
                    'SELECT t.topic_last_post_time, t.topic_id
					FROM ' .
                    TOPICS_TABLE .
                    " t
                    ...
                    (count($ex_fid_ary)
                        ? " AND " .
                            $db->sql_in_set("t.forum_id", $ex_fid_ary, true)
                        : "") .
                    '
            ...
            case "unanswered":
                ...
                $sql_sort =
                    $sort_key == "f"
                        ? " AND f.forum_id = p.forum_id " . $sql_sort
                        : $sql_sort;
                ...
                $template->assign_var(
                    ...
                    $config["load_anon_lastread"]
                        ? append_sid(
                            ...
                            "hash=" .
                                generate_link_hash("global") .
                                "&amp;mark=forums&amp;mark_time=" .
                                time()
                        )
                        : ""
                );
            ...
            case "newposts":
                ...
                // force sorting
                $show_results =
                    $request->variable("sr", "topics") == "posts"
                        ? "posts"
                        : "topics";
                $sort_key = "t";
                $sort_dir = "d";
     ...
     * @since 3.1.7-RC1
     */
    $vars = ["l_search_title", "search_id", "show_results", "sql"];
    extract(
        $phpbb_dispatcher->trigger_event(
    ...
    if ($search_id) {
        ...
        if ($total_match_count) {
            ...
            $start = $pagination->validate_start(
                $start,
                $per_page,
                $total_match_count
            );
    ...
    if ($search->get_search_query()) {
        $total_match_count = $search->keyword_search(
            $show_results,
            $search_fields,
            $search_terms,
            $sort_by_sql,
    ...
    } elseif (count($author_id_ary)) {
        $firstpost_only =
            $search_fields === "firstpost" || $search_fields == "titleonly"
                ? true
                : false;
    ...
    if (count($id_ary)) {
        $sql_where .= $db->sql_in_set(
            $show_results == "posts" ? "p.post_id" : "t.topic_id",
            $id_ary
        );
        $sql_where .= count($ex_fid_ary)
            ? " AND (" .
                $db->sql_in_set("f.forum_id", $ex_fid_ary, true) .
                " OR f.forum_id IS NULL)"
            : "";
        $sql_where .=
            " AND " .
            ($show_results == "posts"
    ...
    $u_hilit = urlencode(
        htmlspecialchars_decode(str_replace("|", " ", $hilit))
    );
    $u_show_results = "&amp;sr=" . $show_results;
    $u_search_forum = implode("&amp;fid%5B%5D=", $search_forum);

    $u_search = append_sid(
        "{$phpbb_root_path}search.$phpEx",
        $u_sort_param . $u_show_results
    );
    $u_search .= $search_id ? "&amp;search_id=" . $search_id : "";
    $u_search .= $u_hilit
        ? "&amp;keywords=" . urlencode(htmlspecialchars_decode($keywords))
        : "";
    ...
    if ($sql_where) {
        if ($show_results == "posts") {
            ...
            $sql_array = [
                "SELECT" =>
                    "p.*, f.forum_id, f.forum_name, t.*, u.username, u.username_clean, u.user_sig, u.user_sig_bbcode_uid, u.user_colour",
                "FROM" => [
                    POSTS_TABLE => "p",
                ],
                "LEFT_JOIN" => [
            ...
            $sql_from =
                ...
                ($sort_key == "a"
                    ? " LEFT JOIN " .
                        USERS_TABLE .
                        " u ON (u.user_id = t.topic_poster) "
                    : "");
            $sql_select = "t.*, f.forum_id, f.forum_name";

            if ($user->data["is_registered"]) {
                if (
                    $config["load_db_track"] &&
                    $author_id !== $user->data["user_id"]
                ) {
                    $sql_from .=
                        " LEFT JOIN " .
                ...
                if ($config["load_db_lastread"]) {
                    ...
                    $sql_select .=
                        ", tt.mark_time, ft.mark_time as f_mark_time";
                }
            }

            if (
                $config["load_anon_lastread"] ||
                ($user->data["is_registered"] && !$config["load_db_lastread"])
            ) {
                $tracking_topics = $request->variable(
                    $config["cookie_name"] . "_track",
                    "",
                    true,
                    \phpbb\request\request_interface::COOKIE
                );
                $tracking_topics = $tracking_topics
                    ? tracking_unserialize($tracking_topics)
                    : [];
            }
        ...
        $rowset = $attachments = $topic_tracking_info = [];
        ...
        if ($show_results == "topics") {
            ...
            while ($row = $db->sql_fetchrow($result)) {
                ...
                if ($row["topic_status"] == ITEM_MOVED) {
                    $shadow_topic_list[$row["topic_moved_id"]] =
                        $row["topic_id"];
                }
                ...
                if (
                    !isset($forums[$row["forum_id"]]) &&
                    $user->data["is_registered"] &&
                    $config["load_db_lastread"]
                ) {
                    $forums[$row["forum_id"]]["mark_time"] =
                        $row["f_mark_time"];
                }
                $forums[$row["forum_id"]]["topic_list"][] = $row["topic_id"];
                $forums[$row["forum_id"]]["rowset"][$row["topic_id"]] =
                    &$rowset[$row["topic_id"]];
            }
            $db->sql_freeresult($result);
            ...
            if (count($shadow_topic_list)) {
                $sql =
                    ...
                    '
					WHERE ' .
                    $db->sql_in_set("topic_id", array_keys($shadow_topic_list));
                $result = $db->sql_query($sql);
                ...
                while ($row = $db->sql_fetchrow($result)) {
                    ...
                    $row = array_merge($row, [
                        "topic_moved_id" =>
                            $rowset[$orig_topic_id]["topic_moved_id"],
                        "topic_status" =>
                            $rowset[$orig_topic_id]["topic_status"],
            ...
            foreach ($forums as $forum_id => $forum) {
                if (
                    ...
                    $topic_tracking_info[$forum_id] = get_topic_tracking(
                        $forum_id,
                        $forum["topic_list"],
                        $forum["rowset"],
                        [$forum_id => $forum["mark_time"]]
                    );
                } elseif (
                    $config["load_anon_lastread"] ||
                    $user->data["is_registered"]
            ...
            while ($row = $db->sql_fetchrow($result)) {
                 ...
                 * @since 3.2.2-RC1
                 */
                $vars = ["hilit", "row", "u_hilit", "view", "zebra"];
                extract(
                    $phpbb_dispatcher->trigger_event(
                ...
                if (
                    $return_chars == -1 ||
                    utf8_strlen($text_only_message) < $return_chars + 3
                ) {
            ...
            if (count($attach_list)) {
                ...
                foreach ($use_attach_list as $forum_id => $_list) {
                    if (
                        $auth->acl_get("u_download") &&
                        $auth->acl_get("f_download", $forum_id)
                    ) {
            ...
            if (count($attach_list)) {
                $sql =
                    ...
                    '
					WHERE ' .
                    $db->sql_in_set("post_msg_id", $attach_list) .
                    '
						AND in_message = 0
        ...
        if ($hilit) {
            ...
            foreach ($hilit_array as $key => $value) {
                ...
                $hilit_array[$key] = str_replace(
                    "\*",
                    "\w*?",
                    preg_quote($hilit_array[$key], "#")
                );
                $hilit_array[$key] = preg_replace(
                    '#(^|\s)\\\\w\*\?(\s|$)#',
                    '$1\w+?$2',
                    $hilit_array[$key]
                );
        ...
        foreach ($rowset as $row) {
            ...
            $replies =
                $phpbb_content_visibility->get_count(
                    "topic_posts",
                    $row,
                    $forum_id
                ) - 1;
            ...
            $view_topic_url_params =
                "f=$forum_id&amp;t=$result_topic_id" .
                ($u_hilit ? "&amp;hilit=$u_hilit" : "");
            ...
            if ($show_results == "topics") {
                ...
                topic_status(
                    $row,
                    $replies,
                    isset($topic_tracking_info[$forum_id][$row["topic_id"]]) &&
                    $row["topic_last_post_time"] >
                        $topic_tracking_info[$forum_id][$row["topic_id"]]
                        ? true
                        : false,
                    $folder_img,
                    $folder_alt,
                ...
                $u_mcp_queue =
                    $topic_unapproved || $posts_unapproved
                        ? append_sid(
                        ...
                        : "";
                $u_mcp_queue =
                    !$u_mcp_queue && $topic_deleted
                        ? append_sid(
                            "{$phpbb_root_path}mcp.$phpEx",
                            "i=queue&amp;mode=deleted_topics&amp;t=$result_topic_id",
                            true,
                            $user->session_id
                        )
                        : $u_mcp_queue;

                $row["topic_title"] = preg_replace(
                    "#(?!<.*)(?<!\w)(" .
                        $hilit .
                        ")(?!\w|[^<>]*(?:</s(?:cript|tyle))?>)#isu",
                    '<span class="posthilit">$1</span>',
                    $row["topic_title"]
                );
                ...
                $tpl_ary = [
                    ...
                    "TOPIC_AUTHOR_COLOUR" => get_username_string(
                        "colour",
                        $row["topic_poster"],
                        $row["topic_first_poster_name"],
                        $row["topic_first_poster_colour"]
                    ...
                    "LAST_POST_TIME" => $user->format_date(
                        $row["topic_last_post_time"]
                    ),
                    "LAST_VIEW_TIME" => $user->format_date(
                        $row["topic_last_view_time"]
                    ),
                    "LAST_POST_AUTHOR" => get_username_string(
                        "username",
                        $row["topic_last_poster_id"],
                        $row["topic_last_poster_name"],
                        $row["topic_last_poster_colour"]
                    ),
                    "LAST_POST_AUTHOR_COLOUR" => get_username_string(
                        "colour",
                        $row["topic_last_poster_id"],
                        $row["topic_last_poster_name"],
                        $row["topic_last_poster_colour"]
                    ),
                    "LAST_POST_AUTHOR_FULL" => get_username_string(
                        "full",
                        $row["topic_last_poster_id"],
                        $row["topic_last_poster_name"],
                        $row["topic_last_poster_colour"]
                    ...
                    "TOPIC_ICON_IMG" => !empty($icons[$row["icon_id"]])
                        ? $icons[$row["icon_id"]]["img"]
                        : "",
                    "TOPIC_ICON_IMG_WIDTH" => !empty($icons[$row["icon_id"]])
                        ? $icons[$row["icon_id"]]["width"]
                        : "",
                    "TOPIC_ICON_IMG_HEIGHT" => !empty($icons[$row["icon_id"]])
                        ? $icons[$row["icon_id"]]["height"]
                        : "",
                    "ATTACH_ICON_IMG" =>
                        $auth->acl_get("u_download") &&
                        $auth->acl_get("f_download", $forum_id) &&
                    ...
                    "U_LAST_POST" =>
                        ...
                        "#p" .
                        $row["topic_last_post_id"],
                    "U_LAST_POST_AUTHOR" => get_username_string(
                        "profile",
                        $row["topic_last_poster_id"],
                        $row["topic_last_poster_name"],
                        $row["topic_last_poster_colour"]
                    ),
                    "U_TOPIC_AUTHOR" => get_username_string(
                        "profile",
                        $row["topic_poster"],
                        $row["topic_first_poster_name"],
                        $row["topic_first_poster_colour"]
                    ...
                    "U_MCP_REPORT" => append_sid(
                        "{$phpbb_root_path}mcp.$phpEx",
                        "i=reports&amp;mode=reports&amp;t=" . $result_topic_id,
                        true,
                        $user->session_id
                    ),
                ...
                if (
                    isset($zebra["foe"]) &&
                    in_array($row["poster_id"], $zebra["foe"]) &&
                    (!$view || $view != "show" || $post_id != $row["post_id"])
                ) {
                    $template->assign_block_vars("searchresults", [
                        "S_IGNORE_POST" => true,

                        "L_IGNORE_POST" => sprintf(
                            $user->lang["POST_BY_FOE"],
                            $row["username"],
                            "<a href=\"$u_search&amp;start=$start&amp;p=" .
                                $row["post_id"] .
                                "&amp;view=show#p" .
                                $row["post_id"] .
                                '">',
                ...
                if ($row["display_text_only"]) {
                    ...
                    $row["post_text"] = get_context(
                        $row["post_text"],
                        array_filter(explode("|", $hilit), "strlen"),
                        $return_chars
                    );
                    $row["post_text"] = bbcode_nl2br($row["post_text"]);
                } else {
                    $parse_flags =
                        ($row["bbcode_bitfield"] ? OPTION_FLAG_BBCODE : 0) |
                        OPTION_FLAG_SMILIES;
                    $row["post_text"] = generate_text_for_display(
                        $row["post_text"],
                        $row["bbcode_uid"],
                        $row["bbcode_bitfield"],
                        $parse_flags,
                ...
                $tpl_ary = [
                    "POST_AUTHOR_FULL" => get_username_string(
                        "full",
                        $row["poster_id"],
                        $row["username"],
                        $row["user_colour"],
                    ...
                    "POST_AUTHOR" => get_username_string(
                        "username",
                        $row["poster_id"],
                        $row["username"],
                        $row["user_colour"],
                    ...
                    "U_POST_AUTHOR" => get_username_string(
                        "profile",
                        $row["poster_id"],
                        $row["username"],
                        $row["user_colour"],
            ...
            $tpl_ary = array_merge($tpl_ary, [
                ...
                "U_VIEW_FORUM" => append_sid(
                    "{$phpbb_root_path}viewforum.$phpEx",
                    "f=" . $forum_id
                ),
                "U_VIEW_POST" => !empty($row["post_id"])
                    ? append_sid(
                            "{$phpbb_root_path}viewtopic.$phpEx",
                            "f=$forum_id&amp;t=" .
                                $row["topic_id"] .
    ...
    if (
        strpos(html_entity_decode($keywords), '"') !== false &&
        method_exists($search, "supports_phrase_search")
    ) {
        $phrase_search_disabled = $search->supports_phrase_search()
            ? false
            : true;
    }
    ...
    $template->assign_vars([
        ...
        "IGNORED_WORDS" => !empty($common_words)
            ? implode(" ", $common_words)
            : "",
        ...
        "NEWEST_POST_IMG" => $user->img(
            "icon_topic_newest",
            "VIEW_NEWEST_POST"
        ),
        "REPORTED_IMG" => $user->img("icon_topic_reported", "TOPIC_REPORTED"),
        "UNAPPROVED_IMG" => $user->img(
            "icon_topic_unapproved",
            "TOPIC_UNAPPROVED"
        ),
...
$s_forums = "";
$sql =
    'SELECT f.forum_id, f.forum_name, f.parent_id, f.forum_type, f.left_id, f.right_id, f.forum_password, f.enable_indexing, fa.user_id
	FROM ' .
    FORUMS_TABLE .
    ' f
	LEFT JOIN ' .
...
foreach ($rowset as $row) {
    ...
    if (
        ...
        ($row["forum_password"] && !$row["user_id"])
    ) {
    ...
    $selected = in_array($row["forum_id"], $search_forum)
        ? ' selected="selected"'
        : "";
...
$s_characters =
    '<option value="-1">' . $user->lang["ALL_AVAILABLE"] . "</option>";
...
$template->assign_vars([
    "S_SEARCH_ACTION" => append_sid(
        "{$phpbb_root_path}search.$phpEx",
        false,
        true,
        0
    ), // We force no ?sid= appending by using 0
...
if ($auth->acl_get("a_search")) {
    ...
    while ($row = $db->sql_fetchrow($result)) {
        ...
        $template->assign_block_vars("recentsearch", [
            "KEYWORDS" => $keywords,
            "TIME" => $user->format_date($row["search_time"]),

            "U_KEYWORDS" => append_sid(
                "{$phpbb_root_path}search.$phpEx",
                "keywords=" . urlencode(htmlspecialchars_decode($keywords))
            ),
        ]);
